...
| N04 | 图片导入 / 拍照 | 顶部右侧和内容库入口复用同一导入服务；相册旧链路不回退；无相机设备给出降级提示 | ✅ 已实现（T100）：`KCImageImport` 单测（10）+ `KCImageImportService` 决策；content-library 探针覆盖相册来源决策；无相机降级/权限拒绝由 `KCImageImportDecision` 单测确定性覆盖（Xcode 26 模拟器带虚拟相机，noCamera 不再可在模拟器触发）。注：拍照导入、生成线稿分支留 T101 | 相册导入、拍照入口、取消、权限失败和模拟器无相机提示 |
...
| N07 | 玻璃材质视觉基线 | 浮层/左轨/底部 Dock/弹层采用统一 iOS 玻璃（半透明、轻模糊、白高光描边、暖棕阴影、分级圆角）；按钮为半透明白+内高光；选中/强调/色样/禁用/画布纸张保持实色；玻璃不破坏图标文字色块识别 | ✅ 设计基线 + G1 + G2 + G3 已实现（T109，2026-07-10）：`docs/architecture/GLASS_MATERIAL_BASELINE.md`；**G1（系统液态玻璃 `UIGlassEffect`，iOS 26+ + iOS<26 降级）**+ **G2（内容库卡片 + 线稿提取结果卡由"假玻璃"改为统一玻璃入口，顺带解 history 嵌套遮挡）**+ **G3（raised 按钮 0.82 + 白高光内边，折叠按钮/chip 改统一玻璃入口）**已编码；自动验收需覆盖 271 tests / validator / `layout-safe-area`·`system-ui`·`content-library`·`drawing-tools` 双端 runtime；减层(G4)/画纸留边+氛围光(G5) 后续 | iPhone/iPad 空白+深色/高饱和画布下玻璃透出与可读性、内容库打开（玻璃透出，history 不再被实色遮死）、Dock 各状态、Toast/线稿选择器/提取结果卡材质一致性、按钮文字图标可读、色样无偏色 |
