...
| N07 | 玻璃材质视觉基线 | 浮层/左轨/底部 Dock/弹层采用统一 iOS 玻璃（半透明、轻模糊、白高光描边、暖棕阴影、分级圆角）；按钮为半透明白+内高光；选中/强调/色样/禁用/画布纸张保持实色；玻璃不破坏图标文字色块识别 | ✅ 设计基线 + G1 已实现并通过 Codex 审核（T109，2026-07-10）：`docs/architecture/GLASS_MATERIAL_BASELINE.md`；**G1（系统液态玻璃 `UIGlassEffect`，iOS 26+ + iOS<26 降级）已编码**并通过 266 tests / validator / `layout-safe-area`·`system-ui`·`content-library` 双端 runtime，待 commit；按钮玻璃化(G3)/假玻璃→真玻璃(G2)/减层(G4)/画纸留边+氛围光(G5) 后续 | iPhone/iPad 空白+深色/高饱和画布下玻璃透出与可读性、内容库打开（无模糊被遮挡）、Dock 各状态、Toast/线稿选择器/提取结果卡材质一致性、色样无偏色 |
