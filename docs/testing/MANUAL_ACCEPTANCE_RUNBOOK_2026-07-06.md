...
| 方向 | 横屏优先；iPhone 与 iPad 都必须可用 | 自动 smoke 已生成横屏观察图；2026-07-10 用户确认人工验收通过 |
...
> 执行备注：2026-07-06 23:05 已完成双端模拟器、相册测试图和权限重置准备；2026-07-10 用户确认人工验收全部通过，以下结果按人工点验结论收口。

| 编号 | 流程 | 人工步骤 | 预期结果 | iPhone 结果 | iPad 结果 | 证据/备注 |
|---|---|---|---|---|---|---|
| F01 | 启动 | 启动 App，切到横屏，观察首屏。 | App 不崩溃；画布、左侧工具栏、右侧面板、底部 Dock 均可见且无遮挡。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F02 | 画笔 | 依次选择铅笔、钢笔、蜡笔；拖动尺寸滑杆；画连续线条。 | 三种画笔都能绘制；尺寸变化可感知；撤销按钮状态正确。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F03 | 橡皮 | 先画几笔，再切换橡皮 circle/cloud/star，调整尺寸并擦除。 | 三种形状切换有效；擦除区域符合预期；可撤销。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F04 | 填色 | 选择线稿或手动画封闭区域，切到填色后点按区域。 | 封闭区域被填色；未明显污染画布外区域；可撤销。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F05 | 取色 | 在已有颜色区域上使用取色器，再切回画笔绘制。 | 当前色更新为取到的颜色；后续画笔使用该颜色。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F06 | 印章 | 添加印章，拖动、捏合放大/缩小、双指旋转、前移、删除，再撤销/重做。 | 印章选中反馈清楚；真实手势有效；删除和撤销/重做链路正确。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F07 | 颜色面板 | 切换 24/36 色盘，选择多个颜色，观察最近色和当前色。 | 色盘切换正常；选中色高亮正确；最近色顺序更新。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F08 | 自定义色 | 点击 Custom，打开系统取色器并选择颜色。 | 仅有一个 Custom 入口；系统取色器可打开；选择后当前色更新。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F09 | 保存 | 空画布点击保存；画一笔后再保存；查看系统相册。 | 空画布显示“无法保存”；非空画布显示“已保存”；系统相册可看到保存图片或记录环境限制。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F10 | 历史 | 保存后打开历史，进入刚保存记录，再删除记录。 | 缩略图可见；打开后画布恢复；删除后列表刷新。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F11 | 相册导入 | 首次打开相册导入，确认权限弹窗；选择一张照片导入。 | 权限说明为中文；照片进入画布；导入后作为干净会话继续绘制。 | 通过 | 通过 | 2026-07-10 用户确认 |
| F12 | 线稿 | 打开线稿入口，选择任一线稿，继续绘制/填色。 | 弹窗展示正常；线稿加载到画布；后续绘制和填色可用。 | 通过 | 通过 | 2026-07-10 用户确认 |

## 4. 系统能力专项点验

| 能力 | 人工步骤 | 必须记录 | 结果 |
|---|---|---|---|
| Photos 权限弹窗 | 清理权限或首次安装后进入相册导入。 | 弹窗语言、按钮、是否能继续导入。 | 通过；2026-07-10 用户确认 |
| 从相册导入图片 | 选择模拟器或真机相册中的一张图片。 | 图片是否进入画布、是否可继续绘制、导入后撤销/重做是否干净。 | 通过；2026-07-10 用户确认 |
| 保存到系统相册 | 非空画布保存后打开 Photos 检查。 | 是否写入 Photos；如模拟器环境无法确认，记录替代证据和限制。 | 通过；2026-07-10 用户确认 |
| 系统自定义取色器 | 点击 Custom 后选择任意颜色。 | 取色器是否出现、颜色是否回填、最近色是否更新。 | 通过；2026-07-10 用户确认 |
| 印章真实捏合/旋转 | 触控板、鼠标模拟多指或真机双指操作。 | 缩放上下限、旋转流畅度、选中框和编辑按钮是否跟随。 | 通过；2026-07-10 用户确认 |
...
| 项目 | 结论 | 备注 |
|---|---|---|
| iPhone 人工点验 | 通过 | 2026-07-10 用户确认 |
| iPad 人工点验 | 通过 | 2026-07-10 用户确认 |
| 系统能力专项 | 通过 | Photos、相册导入/保存、系统取色器、印章真实手势均已人工确认 |
| 阻塞缺陷 | 0 | 本轮人工验收未反馈阻塞缺陷 |
| 非阻塞缺陷 | 0 | T110 顶部调色盘入口代码变量已收敛为 `paletteButton`，不再作为非阻塞缺陷保留 |
| 是否可交付试用 | 可以 | 本轮自动验收 + 人工验收均已通过 |
