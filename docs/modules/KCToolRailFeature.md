...
- 统一维护取色器按钮的黄色强调色。
- 判断工具按钮是否匹配当前工具模式，并统一应用选中态 / 未选中态背景、边框和阴影强度。
...
- `scripts/validate_project.py` 校验新文件已进入 App target Sources，并防止工具 tuple 配置和工具栏选中态样式回流主控制器；T056 后基础按钮质感来自 `KCEditorUIFactory`，本 Feature 只叠加选中态。
