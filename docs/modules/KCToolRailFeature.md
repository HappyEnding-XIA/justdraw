...
- 提供左侧工具栏当前支持的工具项列表：画笔、橡皮擦、填色、贴纸、取色器。
- 统一维护工具项的稳定 id、`KDToolMode`、SF Symbol 和本地化标题。
- 统一维护取色器按钮的黄色强调色。
- 判断工具按钮是否匹配当前工具模式，并统一应用选中态 / 未选中态外观。
- 输出 `KCToolRailItem` DTO，供 `KCMainViewController` 创建按钮、设置无障碍标识和绑定事件。

## 2. 边界

- 只负责工具栏配置数据和按钮状态外观，不创建 UIKit 控件，不注册 target/action。
- 不负责左侧工具栏尺寸、滚动容器和按钮基础样式；尺寸与基础样式仍由 `KCDeviceLayoutMetrics`、`KCEditorUIFactory` 和控制器布局承担。
- 不负责画布工具行为；按钮点击后仍由 `KCMainViewController.didTapToolButton(_:)` / `selectToolMode(_:)` 协调画布状态、尺寸预览、贴纸编辑按钮和底部画笔 Dock。
...
- `scripts/validate_project.py` 校验新文件已进入 App target Sources，并防止工具 tuple 配置和工具栏选中态样式回流主控制器。
