...
- 集中维护 App 内可复用的 `KCEditorVisualStyle`，统一玻璃态面板、连续圆角、按钮边框、阴影、选中态、禁用态、紧凑按钮和小工具按钮外观。
- 提供 `applySelectableButtonAppearance(...)` 与 `applyActionButtonAvailability(...)`，让工具栏、画笔 Dock、橡皮按钮、印章按钮和画布动作按钮共用同一套状态视觉。
...
- `scripts/validate_project.py` 校验新文件已进入 App target Sources，并守护关键控件创建、控制器委托和 T056/T059/T061 视觉样式入口。

## 4. 验收规则

- 不允许把通用控件样式重新堆回 `KCMainViewController`。
- 不允许绕过 `KCEditorVisualStyle` 新增散落的浮层面板、通用按钮、紧凑按钮、小工具按钮、折叠控件、动作按钮或印章面板 token。
