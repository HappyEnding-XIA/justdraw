...
- 为线稿选择面板生成缩略图，并在 Feature 内用 `NSCache` 复用已渲染缩略图；首次 miss 时由后台缩略图队列渲染，避免弹窗构建时同步画完所有预览图。
...
- `KCLineArtPickerViewController` 的线稿按钮先通过 `lineArtFeature.cachedThumbnailImage(for:)` 读取缓存；缓存 miss 时调用 `prepareThumbnailImage(for:completion:)` 后台渲染，完成后回主线程补齐按钮图片。
- 加载线稿时，`KCMainViewController` 先在主线程读取当前 `canvasSize` 与 `drawingRect`，再通过 `lineArtRenderingQueue` 调用 `lineArtFeature.lineArtImage(for:canvasSize:drawingRect:)` 生成画布图片，完成后回主线程交给 `KCDrawingCanvasView.loadLineArtImage(_:)`。正常选择线稿时不得无条件 `layoutIfNeeded()` 刷新整棵 UI；只有画布 bounds 仍为 zero 时才允许兜底同步布局。
...
- `KCLineArtFeature.thumbnailImage(for:)` 必须先查 `thumbnailCache.object(forKey:)`，miss 后再调用私有渲染方法并 `setObject`；picker UI 不得在按钮构建路径同步调用该方法。
- `KCLineArtFeature.prepareThumbnailImage(for:completion:)` 必须通过 `thumbnailRenderingQueue` 后台生成 miss 缩略图，并在主线程回调。
...
- 线稿加载不得在读取已存在的 `canvasView.bounds.size` 前无条件调用 `layoutIfNeeded()`；同步布局只能作为 bounds 为 zero 的兜底路径。
