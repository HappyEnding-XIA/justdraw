...
- `KCMainViewController` 只保留线稿弹窗展示、按钮点击和替换画布的页面协调。
...
- 线稿按钮缩略图通过 `lineArtFeature.thumbnailImage(for:)` 获取。
- 加载线稿时通过 `lineArtFeature.lineArtImage(for:canvasSize:)` 获取画布图片，再交给 `KCDrawingCanvasView.loadLineArtImage(_:)`。
