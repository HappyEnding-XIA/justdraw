...
- 历史缩略图背景图和前景占位状态必须同时应用到 normal / highlighted / selected / disabled / focused 以及 selected+highlighted、selected+focused 等组合状态；非空槽位使用 `nil` 清掉前景图，用户按下、选中或 iPad focus 缩略图时不得回退到默认图片占位。
- 启动首帧前的初始历史刷新必须传入 `checkDraftExistence: false`，不读取 `draft.png` 文件状态；首帧后再通过 deferred work 恢复草稿和刷新历史。
...
- 禁止已保存缩略图在 loading / highlighted / selected 状态露出默认 photo 占位图；非空 saved 槽位必须清空 UIButton 前景图，只有空槽位和无草稿槽位可以展示占位符。
