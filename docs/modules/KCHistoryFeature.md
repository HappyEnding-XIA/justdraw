...
- `KCMainViewController.refreshHistoryUI()` 已保存缩略图只允许读取 `KCSessionService.cachedThumbnailImage(forSession:)` 的内存缓存；该入口复用当前 `sessions` 中已加载的 metadata，不再按 id 重复查找会话列表。当前页缓存 miss 时必须用透明图兜底并隐藏默认 photo 占位图；如果槽位仍代表同一个会话，保留已显示缩略图，不因一次 cache miss 清空画面，再通过 `preloadVisibleHistoryThumbnailsIfNeeded(_:)` 后台解码并回主线程以 `loadDraftThumbnail: false / preloadThumbnails: false / loadSessions: false` 刷新，避免翻到未缓存页面或点选时阻塞/闪烁，并避免预热回调再次触发同步草稿缩略图或 metadata 读取。
- 历史缩略图背景图和前景透明兜底图必须同时应用到 normal / highlighted / selected / disabled / focused 以及 selected+highlighted、selected+focused 等组合状态；用户按下、选中或 iPad focus 缩略图时不得回退到默认图片占位。
...
- 禁止已保存缩略图在 loading / highlighted / selected 状态露出默认 photo 占位图；非空 saved 槽位必须使用透明兜底图关闭 UIButton 状态回退，只有空槽位和无草稿槽位可以展示占位符。
