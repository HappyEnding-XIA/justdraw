...
- 判断当前是否为紧凑 iPhone 布局。
- 输出右侧面板、底部工具坞、画笔子类型卡片、历史缩略图等固定尺寸和安全区距离。
...
- T115 后底部 Dock 不再预留大类标题宽度，横向空间全部用于当前工具的子类型按钮；T115 视觉返修后 Dock 容器按 3 个子类型按钮收紧，避免右侧空白过大。`bottomDockHorizontalInset`、`brushCardWidth` 和 `brushCardHeight` 继续由本类型集中控制。
- `scripts/validate_project.py` 校验新文件已进入 App target Sources，并守护关键 iPhone/iPad 尺寸值。

## 4. 验收规则

- 不允许把右侧面板、底部工具坞、画笔子类型卡片、历史缩略图的设备尺寸判断重新散落回 `KCMainViewController`。
