...
- 判断 Dock 按钮是否匹配当前工具 / 当前画笔，并应用选中态背景、边框、阴影和轻量缩放；颜色与边框 token 复用 `KCEditorVisualStyle`。
...
- `scripts/validate_project.py` 校验新文件已进入 App target Sources，并防止画笔 tuple 配置、`brushColor` 决策和 Dock 选中态样式回流主控制器；T056/T060 后基础卡片质感和通用视觉 token 来自 `KCEditorUIFactory` / `KCEditorVisualStyle`，本 Feature 只叠加选中态。
