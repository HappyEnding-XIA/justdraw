...
App 层画笔 Dock Feature：集中底部 Dock 的画笔子类型配置，包括画笔 id、`KDBrushStyle`、`KDToolMode`、SF Symbol、强调色和本地化标题。位于 `KidCanvas/Features/Tools/KCBrushDockFeature.swift`，不是独立 SPM target。

## 1. 职责

- 提供底部 Dock 当前支持的画笔子类型列表：铅笔、钢笔、蜡笔。
...
- `buildBottomDock(_:)` 通过 `brushDockFeature.brushItems()` 获取画笔配置，再交给 `toolCardButtonWithSymbolName(...)` 创建卡片按钮；Dock 不再展示“画笔 / Brushes”大类标题，只展示当前工具模式下可切换的具体子类型，容器宽度按三枚按钮收紧，避免右侧残留大块空白。
...
- 不允许在底部 Dock 恢复“画笔 / Brushes”大类标题；主工具分类入口属于左侧工具栏，底部 Dock 只承载当前工具的子类型切换。
- 不允许底部 Dock 在仅展示三枚画笔子类型按钮时保留旧标题时代的大宽度空白；尺寸应继续由 `KCDeviceLayoutMetrics` 集中控制。
