...
App 层保存反馈 Toast 展示器：承接保存成功、保存失败、相册导出失败提示的 UIKit 视图创建、位置约束、图标、文字和动画。位于 `KidCanvas/DesignSystem/KCToastPresenter.swift`，不是独立 SPM target。

## 1. 职责

- 创建保存反馈 Toast 的 `UIVisualEffectView`、系统图标和固定尺寸。
- 根据保存成功、保存失败或相册导出失败选择对应图标、文字与颜色。
...
- 不允许相册导出失败复用“无法保存”文案；该文案只用于 App 内保存失败或空画布保存反馈。
