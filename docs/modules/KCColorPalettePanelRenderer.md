...
- 渲染当前色盘的颜色按钮，维护按钮尺寸、圆角、边框、无障碍标识和网格约束；色盘按钮网格由主控制器在首帧后首次加载，避免启动前创建 24 个颜色按钮。
...
- `reloadPaletteGrid()` 委托 `reloadPaletteGrid(...)` 刷新当前 24/36 色盘按钮；启动路径必须通过 `loadColorControlsAfterStartupIfNeeded()` 延后第一次调用。
