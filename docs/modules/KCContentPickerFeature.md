...
- `KCMainViewController.loadRecentColorsIfNeeded()` 在首帧后的 deferred work 或首次写入最近色前调用 `contentPicker.loadRecentColors()`，保持原有最近色数据兼容且不阻塞首帧。启动期颜色/最近色 UI 当前由 `KCStartupDeferredDelay.colorControls = 0.16` 错峰加载，避免紧贴首帧抢主线程。
