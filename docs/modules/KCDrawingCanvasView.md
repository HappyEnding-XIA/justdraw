...
- T116 起 `draw(_:)` 不再逐帧遍历已提交 `strokes`：工作台底色/氛围光使用 bounds、screen scale 和 interface style 键控的独立缓存，完成内容 raster 在 viewport 变换后绘制，活动笔画单独叠加。Debug `brush-interaction` 验证 300 条历史笔画 viewport 帧不增加 replay/rebuild 计数。
...
- 完成内容只保留一个有界 raster cache；viewport 变化不能逐帧重放历史笔画。内存警告、bounds、屏幕 scale 或界面样式变化必须清理相应缓存。
- 目标阈值：600 采样增量/旧全量比例 `<= 0.35`，追加批次 P95 `<= 8ms`，最大批次 `< 50ms`，300 条历史笔画 viewport 不新增 replay/rebuild，蜡笔偏移比 `<= 0.060001`、aspect ratio `<= 1.35`、几何全为有限值。
- 当前自动化证据：iPhone 17 Pro 增量比例约 `0.0273`、P95 `0.0091ms`、最大 `0.0130ms`；iPad Pro 11 M4 增量比例约 `0.0299`、P95 `0.0150ms`、最大 `0.0380ms`；双端均通过 `brush-interaction`。
- iPad Pro 11 M4 模拟器在 300 条历史笔画的 Debug 合成观测约 `22.7 FPS`，模拟器帧率不作为老款实体 iPad 的最终结论；仍需在目标老款 iPad 上确认最低 `30 FPS` 和无 `>=50ms` 主线程停顿。
