...
- T093 已在 `KCDrawingEngine` 建立纯 dab 引擎（`KCBrushDabGenerator` / `KCBrushInputSample` / `KCBrushPreset` / `KCBrushDab`，见 `docs/modules/KCDrawingEngine.md` 第 4 节）；UIKit 层把高保真采样（coalesced touches、force、altitude/azimuth）与 dab 序列光栅化接入 `drawStroke(_:)` 的工作在 T094 完成，当前画布仍消费 `RenderProfile`。
