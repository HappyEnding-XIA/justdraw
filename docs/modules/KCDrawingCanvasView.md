...
- T094 起铅笔/蜡笔走 dab 渲染（引擎见 `docs/modules/KCDrawingEngine.md` 第 4 节）：`touchesBegan/Moved/Ended` 对每个 coalesced touch 采集 `KCBrushInputSample`（point/timestamp/pressure/altitude/azimuth/isPencil）存入 `KDStroke.samples`；`drawStroke(_:)` 对带 `samples` 的铅笔/蜡笔分支调用 `drawingEngine.brushDabs(...)` 生成 `[KCBrushDab]`，再由 `drawDabs` 按 brush-tip mask 批量光栅化。**T111 起 `brushDabs` 传入 `stroke.lineWidth`**（`resolvedDabs(for:)`），引擎 preset 经 `scaledForLineWidth` 据此缩放 dab 半径，故铅笔/蜡笔的尺寸 slider 真正生效；`stroke.cachedDabs` 按 stroke 缓存（lineWidth 在 stroke 创建时由 `currentLineWidth` 落定，尺寸变化只影响新 stroke）。brush-tip mask 缓存在 `NSCache`（键为 brushStyle/textureSeed/颜色），颗粒纹理由 `kcBrushDabMix` 按 `textureSeed` 确定性生成，故同 stroke 重绘（undo/redo）像素一致。钢笔仍走 path smoothing（`renderedLineWidth = stroke.lineWidth`），橡皮/填色/取色/贴纸不变；无 `samples` 的旧 stroke 回落 path 渲染。dirty rect 由 `KCBrushDab.bounds(inset:)` 取并集局部刷新（`strokeRenderBounds`），不退回全量 `setNeedsDisplay()`。
