...
- 当前：浮层、左轨、底部 Dock、内容库卡片、线稿提取结果卡、线稿选择器和 Toast 均收敛到 `KCEditorVisualStyle` 统一玻璃入口；iOS 26+ 使用 `UIGlassEffect`，低版本降级为系统材质。按钮使用 0.82 半透明白与顶部内高光，尺寸面板已移除额外半透明 shell，画布屏幕呈现增加保守纸张留边和轻工作台氛围光。
- 边界：玻璃统一收敛进 `KCEditorVisualStyle`/`KCEditorUIFactory`，禁止内联散写 `UIVisualEffectView`、不引入 CIFilter 自定义模糊；调色盘色样/选中强调按钮/禁用态/画布纸张保持实色（保儿童识别与准确辨色）。T109 G1-G5 已全部实现：统一液态玻璃入口、卡片真玻璃、按钮玻璃化、尺寸面板减层，以及画纸留边与工作台氛围光均已落地；后两项只影响屏幕呈现，不改变作品数据。
...
- `KCImageImportService`（T100 落地，T104 收口）统一相册与拍照导入：`KCDomain.KCImageImportDecision` 保持 UIKit-free；相册选择由 App 层 `PHPickerViewController` 承担，单选图片且无需主动申请完整相册读权限；拍照继续使用 `UIImagePickerController(.camera)` 和 `AVCaptureDevice` 授权。顶栏、内容库和“从照片生成线稿”入口复用同一异步归一化、generation guard、草稿保护与失败反馈链路。
