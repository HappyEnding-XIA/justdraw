...
- **T101 `KCLineArtExtracting`（已完成）**：离线图片生成线稿。`KCDomain.KCLineArtExtraction`（结果 + `KCLineArtQuality` good/marginal/poor + `KCLineArtExtracting` 协议）+ `KCDrawingEngine.KCLineArtExtractor`（Core Image pipeline：灰度→降噪→边缘→反相→高对比阈值化→白底 PNG/缩略图 + 输入亮度均值/标准差质量评估，单测）+ App 确认流（使用这张线稿/重新生成/取消，poor 强制重选）。结果复用 T099 我的线稿存储（`.photoExtraction`）。详见 `docs/modules/KCLineArtExtraction.md`。
...
| `KCLineArtExtraction` | Core | 离线图片生成线稿 pipeline（T101 已落地） |
