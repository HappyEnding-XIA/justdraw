...
/// 内容库（Content Library）的 UIKit-free 纯逻辑模型——T098 的状态边界。
///
/// 内容库按需展开，统一承载官方线稿、我的线稿、历史作品三个分区（导入结果分区
/// 预留给 T100/T101）。本类型只描述分区能力、分区展示状态与空态判定，不依赖 UIKit、
/// 不读取会话文件、不生成线稿、不持有系统 picker；真正的 UIKit 面板编排与数据装配
/// 由 App 层 `KCContentLibraryFeature` / `KCMainViewController` 完成。
