...
final class HexColorTests: XCTestCase {
    ...
    func testRoundTripsAlpha() {
        let original = HexColor(red: 0.2, green: 0.4, blue: 0.6, alpha: 0.5)
        let parsed = HexColor(hex: original.hex)
        XCTAssertEqual(original.hex, parsed?.hex)
    }

    func testRejectsMalformedInput() {
        XCTAssertNil(HexColor(hex: "nope"))
        XCTAssertNil(HexColor(hex: "#12345"))
        XCTAssertNil(HexColor(hex: "#GGGGGG"))
    }

    func testRgba8MatchesPrototypeRounding() {
        // Prototype uses lrint(component * 255): 0.94 -> 240.
        let color = HexColor(red: 0.94, green: 0.43, blue: 0.45, alpha: 1.0)
    ...
    func testCodableRoundTrip() throws {
        ...
        let original = HexColor(red: 0.5, green: 0.25, blue: 0.125, alpha: 1.0)
    ...
    func testClampsOutOfRangeComponents() {
        let color = HexColor(red: 2, green: -1, blue: 0.5, alpha: 1)
