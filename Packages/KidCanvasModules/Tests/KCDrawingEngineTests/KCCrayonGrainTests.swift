...
final class CrayonGrainTests: XCTestCase {
    ...
    // grainBounds {-5,-5,110,60}；spacing = max(3.4, 2.5) = 3.4；
    // columnCount = ceil(110/3.4) = 33；rowCount = ceil(60/3.4) = 18；
    // dash count = (18+1) * (33+1) = 646；dashWidth = max(1.6, 2.8) = 2.8。
    ...
    func testDashCountAndConstantWidthForReferenceBounds() {
        ...
        for dash in dashes {
            XCTAssertEqual(dash.lineWidth, 2.8, accuracy: 1e-9)
        }
    }

    func testFirstDashGeometryMatchesPrototype() {
        // row 0, column 0：seed 0 → jitter (-1.74, -1.50)；center (-6.74, -6.50)；
        // dashLength 2.8；y offset +1.2（偶数 seed）。
        let dashes = KCCrayonGrain.dashes(pathBounds: CrayonGrainTests.referenceBounds,
                                          lineWidth: CrayonGrainTests.referenceLineWidth)
        let first = dashes[0]
        XCTAssertEqual(first.start.x, -8.14, accuracy: 1e-9)
        XCTAssertEqual(first.start.y, -6.50, accuracy: 1e-9)
        XCTAssertEqual(first.end.x, -5.34, accuracy: 1e-9)
        XCTAssertEqual(first.end.y, -5.30, accuracy: 1e-9)
    }

    func testDashGeometryAtRowColumnMatchesPrototype() {
        // row 1, column 1（行优先索引 34 + 1 = 35）：seed 54。
        // jitter (1.16, 0.50)；center (-0.44, -1.10)；
        // dashLength = 10 * (0.28 + 4*0.055) = 5.0；y offset +1.2。
        let dashes = KCCrayonGrain.dashes(pathBounds: CrayonGrainTests.referenceBounds,
                                          lineWidth: CrayonGrainTests.referenceLineWidth)
        let dash = dashes[35]
        XCTAssertEqual(dash.start.x, -2.94, accuracy: 1e-9)
        XCTAssertEqual(dash.start.y, -1.10, accuracy: 1e-9)
        XCTAssertEqual(dash.end.x, 2.06, accuracy: 1e-9)
        XCTAssertEqual(dash.end.y, 0.10, accuracy: 1e-9)
    }

    func testGridRespectsColumnAndRowCaps() {
        // 巨大的 bounds 配合较小的 spacing 会同时触发两个上限：220 列 × 180 行。
        // lineWidth 4 → spacing = max(3.4, 1.0) = 3.4；grainBounds 每侧扩展 2。
    ...
    func testDashWidthScalesForWideCrayonLines() {
        ...
        XCTAssertFalse(dashes.isEmpty)
        for dash in dashes {
            XCTAssertEqual(dash.lineWidth, 6.72, accuracy: 1e-9)
        }
    }
    ...
    func testProductizedGrainIsDenseEnoughForVisibleCrayonTexture() {
        ...
        XCTAssertGreaterThanOrEqual(dashes.count, 850)
        XCTAssertLessThanOrEqual(dashes.count, 950)
        XCTAssertGreaterThanOrEqual(dashes[0].lineWidth, 4.4)
    }
    ...
    func testMediumCrayonGrainKeepsVisiblePaperToothDensity() {
        ...
        // 中等线宽不能密到糊平纸纹；颗粒要更粗、更少，给纸齿留出可见空隙。
        XCTAssertGreaterThanOrEqual(dashes.count, 850)
        XCTAssertLessThanOrEqual(dashes.count, 950)
    }
}
