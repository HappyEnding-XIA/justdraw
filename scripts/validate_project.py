...
def app_feature_checks(
    ...
    checks.append(require_regex(session_store_bridge_text, r"func preloadThumbnailImages\(forSessionIds sessionIds: \[String\], completion: \(\(\) -> Void\)\? = nil\)[\s\S]*thumbnailImageCache\.object[\s\S]*thumbnailPreloadInFlight[\s\S]*thumbnailPreloadQueue\.async[\s\S]*DispatchQueue\.main\.async", "History thumbnail preloading skips cached/in-flight ids before background decode and can refresh UI on completion"))
