...
def app_feature_checks(
    ...
    checks.append(require_regex(main_text, r"func restoreDraftIfNeeded\(\)[\s\S]*self\.draftPersistenceQueue\.async[\s\S]*loadDraftData\(\)[\s\S]*UIImage\(data: data\)[\s\S]*DispatchQueue\.main\.async", "Startup draft restore reads and decodes draft off the main thread"))
