...
def app_feature_checks(
    ...
    checks.append(require_text(stroke_render_math_text, "lineWidth * 1.28 * pressure", "Crayon render metrics stay broad without relying only on width"))
    checks.append(require_text(stroke_render_math_text, "min(0.30, 0.14 + pressure * 0.12)", "Pencil base alpha stays below solid ink"))
    checks.append(require_text(stroke_render_math_text, "min(0.18, 0.06 + pressure * 0.10)", "Crayon base alpha stays below solid ink"))
    ...
    checks.append(require_text(stroke_render_math_text, "widthMultiplier: 1.70", "Crayon profile includes a wide wax smear layer"))
    checks.append(require_text(stroke_render_math_text, "grainAlpha: 0.94", "Crayon grain alpha stays visibly textured"))
    ...
    checks.append(require_text(crayon_grain_text, "lineWidth * 0.09", "Crayon grain scales visibly on wide strokes"))
    checks.append(require_text(drawing_bridge_text, "max(0.9, lineWidth * 0.09)", "Crayon grain dash width used by UIKit matches the engine grain model"))
    ...
    checks.append(require_regex(canvas_text, r"func rasterImageExcludingStickers\(\) -> UIImage[\s\S]*if let cachedImage = nonStickerRasterCacheImage[\s\S]*nonStickerRasterCacheBounds\.equalTo\(bounds\)[\s\S]*nonStickerRasterCacheScale[\s\S]*return cachedImage[\s\S]*UIGraphicsImageRenderer\(bounds: bounds, format: format\)[\s\S]*cacheNonStickerRasterImage\(image, scale: scale\)", "Flood fill reuses a bounds/scale-matched non-sticker raster before redrawing the full canvas"))
    ...
    checks.append(require_regex(main_text, r"func scheduleStartupDeferredWorkIfNeeded\(\)[\s\S]*didScheduleStartupDeferredWork[\s\S]*DispatchQueue\.main\.async[\s\S]*self\.loadColorControlsAfterStartupIfNeeded\(\)[\s\S]*self\.refreshHistorySessionsAsync\(loadDraftThumbnail: false\)[\s\S]*self\.restoreDraftIfNeeded\(\)", "Startup color controls, draft restore, and metadata/thumbnails are deferred until after first appearance"))
