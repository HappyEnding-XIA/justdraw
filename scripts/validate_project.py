...
def app_feature_checks(
    ...
    checks.append(require_text(stroke_render_math_text, "min(0.075, 0.020 + pressure * 0.052)", "Crayon base alpha stays behind chunky wax texture"))
    ...
    checks.append(require_text(main_text, "static let stickerButtons: TimeInterval = 0.24", "Sticker buttons no longer wait over half a second after first appearance"))
    ...
    checks.append(forbid_text(editor_ui_factory_text, "adjustsImageWhenHighlighted", "History thumbnail buttons avoid deprecated highlight image adjustment"))
    ...
    checks.append(require_regex(editor_ui_factory_text, r"historyThumbPlaceholderStates[\s\S]*\.highlighted\.union\(\.selected\)[\s\S]*func historyThumbButton\(\) -> UIButton[\s\S]*for state in Self\.historyThumbPlaceholderStates[\s\S]*button\.setImage\(Self\.historySlotPlaceholderImage\(\), for: state\)", "History thumbnail factory writes the placeholder across highlighted/selected/disabled states"))
