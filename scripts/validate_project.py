...
def app_feature_checks(
    ...
    checks.append(require_regex(canvas_text, r"func touchesEnded[\s\S]*strokes\.append\(activeStroke\)[\s\S]*invalidateNonStickerRasterCache\(\)", "Committing a stroke invalidates the flood-fill raster cache"))
    ...
    checks.append(require_regex(canvas_text, r"func rasterImageExcludingStickers\(includeActiveStroke: Bool = false\) -> UIImage[\s\S]*if !includeActiveStroke,[\s\S]*let cachedImage = nonStickerRasterCacheImage[\s\S]*nonStickerRasterCacheBounds\.equalTo\(bounds\)[\s\S]*nonStickerRasterCacheScale[\s\S]*return cachedImage[\s\S]*UIGraphicsImageRenderer\(bounds: bounds, format: format\)[\s\S]*if !includeActiveStroke \{[\s\S]*cacheNonStickerRasterImage\(image, scale: scale\)", "Flood fill and snapshot reuse a bounds/scale-matched non-sticker raster before redrawing the full canvas"))
