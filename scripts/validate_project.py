...
def app_feature_checks(
    ...
    checks.append(require_text(stroke_render_math_text, "min(0.055, 0.014 + pressure * 0.041)", "Crayon base alpha stays behind chunky wax texture"))
    ...
    checks.append(require_text(crayon_grain_text, "let spacing = max(3.0, lineWidth * 0.20)", "Crayon grain uses denser wax texture spacing"))
    checks.append(require_text(crayon_grain_text, "lineWidth * 0.24", "Crayon grain scales visibly on wide strokes"))
    checks.append(require_text(drawing_bridge_text, "max(1.4, lineWidth * 0.24)", "Crayon grain dash width used by UIKit matches the engine grain model"))
    ...
    checks.append(require_text(main_text, "static let colorControls: TimeInterval = 0.16", "Palette/recent-color startup work waits until the first interaction window is responsive"))
    checks.append(require_text(main_text, "static let restoreDraft: TimeInterval = 0.08", "Draft restore starts soon after the first frame"))
    checks.append(require_text(main_text, "static let historySessions: TimeInterval = 0.32", "History metadata loading is deferred beyond the immediate launch burst"))
    checks.append(require_text(main_text, "static let stickerButtons: TimeInterval = 0.48", "Sticker buttons load after critical startup history/draft work"))
