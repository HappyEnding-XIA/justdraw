...
def app_feature_checks(
    ...
    checks.append(require_text(refresh_history_text, "cachedThumbnailImage(forSessionId:", "History refresh uses cache-only saved thumbnails"))
    ...
    checks.append(require_regex(refresh_history_text, r"else \{[\s\S]*cachedThumbnailImage\(forSessionId: session\.identifier\)[\s\S]*Self\.setHistoryButtonPlaceholderVisible\(false, on: button\)", "Saved history thumbnails do not show the default photo placeholder while loading"))
