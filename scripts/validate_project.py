...
def app_feature_checks(
    ...
    checks.append(require_text(main_text, "self.refreshHistoryUI(loadDraftThumbnail: false, preloadThumbnails: false)", "Initial history refresh skips thumbnail preloading before the first frame"))
    checks.append(require_regex(main_text, r"func scheduleStartupDeferredWorkIfNeeded\(\)[\s\S]*didScheduleStartupDeferredWork[\s\S]*DispatchQueue\.main\.async[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false\)[\s\S]*self\.restoreDraftIfNeeded\(\)", "Startup draft restore and thumbnail preloading are deferred until after first appearance"))
    checks.append(require_text(main_text, "self.refreshHistoryUI(loadDraftThumbnail: false)", "Startup history refresh does not synchronously decode draft thumbnails"))
