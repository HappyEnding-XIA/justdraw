...
def app_feature_checks(
    ...
    checks.append(require_regex(main_text, r"func scheduleStartupDeferredWorkIfNeeded\(\)[\s\S]*scheduleStartupDeferredTask\(after: 0\.05\)[\s\S]*loadColorControlsAfterStartupIfNeeded\(\)[\s\S]*scheduleStartupDeferredTask\(after: 0\.18\)[\s\S]*refreshHistorySessionsAsync\(loadDraftThumbnail: false\)[\s\S]*scheduleStartupDeferredTask\(after: 0\.36\)[\s\S]*restoreDraftIfNeeded\(\)", "Startup color controls, history metadata, and draft restore are staggered after first appearance"))
    checks.append(require_regex(main_text, r"func scheduleStartupDeferredWorkIfNeeded\(\)[\s\S]*scheduleStartupDeferredTask\(after: 0\.62\)[\s\S]*loadStickerButtonsAfterStartupIfNeeded\(\)[\s\S]*func scheduleStartupDeferredTask\(after delay: TimeInterval", "Startup sticker button row is deferred to a later staged task"))
