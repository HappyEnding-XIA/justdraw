...
def app_feature_checks(
    ...
    checks.append(require_text(stroke_render_math_text, "lineWidth * 1.84 * pressure", "Crayon render metrics are visibly thicker than pen"))
    checks.append(require_text(stroke_render_math_text, "min(0.30, 0.14 + pressure * 0.12)", "Pencil base alpha stays below solid ink"))
    checks.append(require_text(stroke_render_math_text, "min(0.30, 0.18 + pressure * 0.10)", "Crayon base alpha stays below solid ink"))
    ...
    checks.append(require_text(stroke_render_math_text, "widthMultiplier: 1.32", "Crayon profile includes a wide wax smear layer"))
    checks.append(require_text(stroke_render_math_text, "grainAlpha: 0.88", "Crayon grain alpha stays visibly textured"))
    ...
    checks.append(require_regex(session_store_bridge_text, r"func loadDraftImage\(\)[\s\S]*if let draftImageCache[\s\S]*return draftImageCache[\s\S]*draftImageCache = image", "Draft image lookup uses cache before decoding from disk"))
    ...
    checks.append(require_text(refresh_history_text, "let draftImage = self.sessionStore.draftThumbnailImage()", "History refresh uses the lightweight draft thumbnail instead of loading the full draft image"))
    ...
    checks.append(require_regex(main_text, r"refreshHistoryUI\(\)[\s\S]*self\.preloadAdjacentHistoryThumbnails\(\)", "History refresh schedules adjacent thumbnail preloading after visible slots"))
