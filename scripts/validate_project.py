...
def app_feature_checks(
    ...
    checks.append(require_regex(canvas_text, r"func drawCrayonGrain[\s\S]*let grainPath = UIBezierPath\(\)[\s\S]*grainPath\.stroke\(\)", "Canvas batches crayon grain dashes into a single path stroke"))
    ...
    checks.append(require_regex(main_text, r"func setHistoryButtonPlaceholderVisible\(_ visible: Bool, on button: UIButton\)[\s\S]*let image = visible \? Self\.historySlotPlaceholderImage\(\) : nil[\s\S]*for state in Self\.historyThumbnailImageStates[\s\S]*button\.setImage\(image, for: state\)[\s\S]*button\.imageView\?\.isHidden = !visible", "History thumbnail placeholder is explicitly hidden for saved-artwork slots"))
    ...
    checks.append(require_text(main_text, "self.refreshHistoryUI(loadDraftThumbnail: false, preloadThumbnails: false, loadSessions: false)", "Initial history refresh skips metadata loading and thumbnail preloading before the first frame"))
