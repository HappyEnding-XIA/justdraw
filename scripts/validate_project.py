...
def app_feature_checks(
    ...
    checks.append(require_regex(main_text, r"func openSession\(_ session: KCSessionMetadata, completion: \(\(Bool\) -> Void\)\? = nil\)[\s\S]*let generation = self\.nextArtworkLoadGeneration\(\)[\s\S]*self\.artworkLoadingQueue\.async[\s\S]*artworkData\(forSession: session\)[\s\S]*UIImage\(data: data\)[\s\S]*DispatchQueue\.main\.async[\s\S]*guard self\.artworkLoadGeneration == generation[\s\S]*self\.canvasView\.restoreCanvas\(with: image\)", "Opening saved artwork reads and decodes the full image off the main thread before restoring"))
    ...
    checks.append(require_regex(main_text, r"func didTapDraftThumb\(\)[\s\S]*let generation = self\.nextArtworkLoadGeneration\(\)[\s\S]*self\.draftPersistenceQueue\.async[\s\S]*loadDraftData\(\)[\s\S]*UIImage\(data: data\)[\s\S]*DispatchQueue\.main\.async[\s\S]*guard self\.artworkLoadGeneration == generation[\s\S]*self\.canvasView\.restoreCanvas\(with: draftImage\)", "Opening a draft reads and decodes the full draft off the main thread before restoring"))
    ...
    checks.append(require_regex(main_text, r"func restoreDraftIfNeeded\(\)[\s\S]*let generation = self\.nextArtworkLoadGeneration\(\)[\s\S]*self\.draftPersistenceQueue\.async[\s\S]*loadDraftData\(\)[\s\S]*UIImage\(data: data\)[\s\S]*DispatchQueue\.main\.async[\s\S]*guard self\.artworkLoadGeneration == generation", "Startup draft restore reads and decodes draft off the main thread and skips stale results"))
