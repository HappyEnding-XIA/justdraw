...
def app_feature_checks(
    ...
    checks.append(require_text(main_text, "private let sessionEncodingQueue", "Formal save has a dedicated encoding queue"))
    checks.append(require_text(main_text, "private var sessionSaveGeneration: Int = 0", "Formal save tracks stale background encoding work"))
    checks.append(require_regex(main_text, r"func didTapSaveSession[\s\S]*self\.sessionEncodingQueue\.async[\s\S]*encodedArtworkData\(from: snapshot\)[\s\S]*DispatchQueue\.main\.async", "Formal save encodes artwork data off the main thread"))
    checks.append(require_regex(main_text, r"DispatchQueue\.main\.async[\s\S]*guard self\.sessionSaveGeneration == generation[\s\S]*finishSavingSession", "Formal save returns through a stale-result guard before mutating UI/session state"))
    checks.append(require_regex(main_text, r"func finishSavingSession[\s\S]*saveArtwork\([\s\S]*pngData: pngData,[\s\S]*thumbnailJPEGData: thumbnailJPEGData", "Formal save writes pre-encoded artwork data through the session service"))
