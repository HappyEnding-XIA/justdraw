...
def app_feature_checks(
    ...
    checks.append(require_text(stroke_render_math_text, "lineWidth * 1.52 * pressure", "Crayon render metrics are visibly thicker than pen"))
    checks.append(require_text(stroke_render_math_text, "min(0.30, 0.14 + pressure * 0.12)", "Pencil base alpha stays below solid ink"))
    checks.append(require_text(stroke_render_math_text, "min(0.24, 0.10 + pressure * 0.12)", "Crayon base alpha stays below solid ink"))
    ...
    checks.append(require_text(stroke_render_math_text, "widthMultiplier: 1.52", "Crayon profile includes a wide wax smear layer"))
    ...
    checks.append(require_text(crayon_grain_text, "lineWidth * 0.065", "Crayon grain scales visibly on wide strokes"))
    ...
    checks.append(require_regex(session_store_bridge_text, r"func cachedDraftThumbnailImage\(\) -> UIImage\?[\s\S]*return draftThumbnailCache", "Session service exposes cache-only draft thumbnail lookup"))
    checks.append(require_regex(session_store_bridge_text, r"func cacheLoadedDraftImage\(_ image: UIImage\)[\s\S]*draftImageCache = image[\s\S]*draftThumbnailCache = Self\.generateThumbnail\(from: image\)", "Session service can cache asynchronously restored drafts"))
    ...
    checks.append(require_regex(refresh_history_text, r"else \{[\s\S]*cachedThumbnailImage\(forSessionId: session\.identifier\)[\s\S]*button\.imageView\?\.isHidden = true", "Saved history thumbnails do not show the default photo placeholder while loading"))
    ...
    checks.append(require_regex(main_text, r"func preserveUnsavedActiveSessionDraftIfNeeded[\s\S]*self\.activeSession != nil && !self\.activeSessionHasUnsavedChanges[\s\S]*return false[\s\S]*self\.activeDraftMatchesCanvas && self\.sessionStore\.hasDraft\(\)[\s\S]*return true[\s\S]*self\.canvasFeature\.hasVisibleContent\(self\.canvasView\)[\s\S]*self\.sessionStore\.saveDraftImage\(snapshot\)[\s\S]*self\.activeDraftMatchesCanvas = saved[\s\S]*return saved", "Visible new canvases and dirty saved sessions reuse a current draft before synchronously saving"))
