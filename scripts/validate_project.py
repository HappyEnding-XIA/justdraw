...
def app_feature_checks(
    ...
    checks.append(require_text(brush_sticker_panel_text, "dots.setContentCompressionResistancePriority(.defaultLow, for: .horizontal)", "Brush size dots can compress inside the narrow right panel instead of logging constraint conflicts"))
    checks.append(require_regex(brush_sticker_panel_text, r"let widthConstraint = dot\.widthAnchor\.constraint\(equalToConstant: size\)[\s\S]*widthConstraint\.priority = \.defaultHigh", "Brush size dot width constraints are high priority instead of required in compact right-panel layouts"))
    checks.append(require_regex(brush_sticker_panel_text, r"let heightConstraint = dot\.heightAnchor\.constraint\(equalToConstant: size\)[\s\S]*heightConstraint\.priority = \.defaultHigh", "Brush size dot height constraints are high priority instead of required in compact right-panel layouts"))
