...
def app_feature_checks(
    ...
    checks.append(require_regex(main_text, r"func invalidateDraftSaveTimer[\s\S]*draftSaveGeneration \+= 1", "Invalidating draft saves cancels stale background writes"))
    ...
    checks.append(require_regex(main_text, r"func saveDraftIfNeeded[\s\S]*self\.draftPersistenceQueue\.async[\s\S]*snapshot\.pngData\(\)[\s\S]*DispatchQueue\.main\.async", "Draft autosave encodes PNG off the main thread"))
    checks.append(require_regex(main_text, r"DispatchQueue\.main\.async[\s\S]*guard self\.draftSaveGeneration == generation[\s\S]*let saved = self\.sessionStore\.saveDraftData\(pngData: pngData, cachedImage: snapshot\)[\s\S]*self\.activeDraftMatchesCanvas = saved", "Draft autosave writes only after stale-result guard and marks the draft current only on success"))
