...
def delivery_acceptance_checks():
    ...
    checks.append(require_text(runtime_acceptance_text, "/usr/bin/python3", "Runtime acceptance script defaults to the macOS system Python when available"))
    checks.append(forbid_text(runtime_acceptance_text, "| python3 -c", "Runtime acceptance script does not hardcode python3 for simctl JSON parsing"))
    ...
    checks.append(require_text(runtime_smoke_text, "PYTHON_BIN", "Runtime smoke test allows a stable Python interpreter override"))
    checks.append(require_text(runtime_smoke_text, "/usr/bin/python3", "Runtime smoke test defaults to the macOS system Python when available"))
    checks.append(forbid_text(runtime_smoke_text, "| python3 -c", "Runtime smoke test does not hardcode python3 for simctl JSON parsing"))
...
def canvas_viewport_checks():
    ...
    checks.append(require_text(main_text, "editorUIFactory.restoreViewportButton(symbolName: \"arrow.down.right.and.arrow.up.left\")", "Restore-view button uses a clear high-contrast dedicated style"))
    checks.append(forbid_text(main_text, "restoreViewportButton = self.iconButtonWithSymbolName", "Restore-view button does not reuse low-contrast top toolbar button style"))
...
def content_library_checks():
    ...
    if panel_file.exists():
        ...
        checks.append(forbid_text(text, "cardView.layer.borderColor = KCEditorVisualStyle.borderColor", "Content library card border is now provided by the glass surface, not cardView (T109 G2)"))
...
def custom_line_art_checks():
    ...
    if grid_file.exists():
        ...
        checks.append(require_text(text, "withRenderingMode(.alwaysOriginal)", "My-line-art thumbnails render as original images instead of UIButton template masks (T113)"))
        checks.append(require_text(text, "imageView?.contentMode = .scaleAspectFit", "My-line-art thumbnails keep aspect-fit display inside grid cells (T113)"))
...
def image_import_checks():
    ...
    if service_file.exists():
        ...
        checks.append(forbid_text(text, "PHPhotoLibrary.authorizationStatus", "Image import service no longer requests full photo-library authorization for import"))
    ...
    checks.append(require_text(picking_text, "PHPickerViewController", "Editor presents PHPicker for photo-library import"))
    checks.append(require_text(picking_text, "configuredPhotoLibraryPickerConfiguration", "Photo-library picker configuration is centralized"))
    checks.append(require_text(picking_text, "selectionLimit = 1", "Photo-library picker limits selection to a single image"))
    checks.append(require_text(picking_text, "filter = .images", "Photo-library picker filters images only"))
    ...
    checks.append(require_text(picking_text, "KCEditorVisualStyle.makeGlassEffectView", "Line-art extraction result card uses the unified glass entry (T109 G2)"))
    checks.append(forbid_text(picking_text, "cardView.backgroundColor = UIColor(white: 1.0, alpha: 0.98)", "Line-art extraction card no longer uses the old 0.98 fake-glass background (T109 G2)"))
    checks.append(forbid_text(picking_text, "cardView.layer.borderColor = KCEditorVisualStyle.borderColor", "Line-art extraction card border is now provided by the glass surface, not cardView (T109 G2)"))
...
def require_count_exact(text, needle, expected, message):
    count = text.count(needle)
    if count != expected:
        return fail(f"{message}: found {count}, expected {expected}")
    return ok(f"{message}: found {count}")
...
def forbid_regex(text, pattern, message, flags=0):
...
def app_feature_checks(
    ...
    checks = []
    refresh_history_start = main_text.find("func refreshHistoryUI(")
    refresh_history_end = main_text.find("private func preloadAdjacentHistoryThumbnails()", refresh_history_start)
    refresh_history_text = (
        main_text[refresh_history_start:refresh_history_end]
    ...
    did_tap_brush_button_start = main_text.find("func didTapBrushButton(")
    did_tap_brush_button_end = main_text.find("func didTapStickerButton(", did_tap_brush_button_start)
    did_tap_brush_button_text = (
        main_text[did_tap_brush_button_start:did_tap_brush_button_end]
        if did_tap_brush_button_start >= 0 and did_tap_brush_button_end > did_tap_brush_button_start
        else ""
    )
    ...
    checks.append(require_text(dab_preset_text, "func scaledForLineWidth(_ lineWidth: Double) -> KCBrushPreset", "Brush preset scales radius by user lineWidth so pencil/crayon size takes effect (T111)"))
    checks.append(require_text(dab_preset_text, "var referenceLineWidth: Double", "Brush preset declares a per-style reference lineWidth for size scaling (T111)"))
    checks.append(require_text(dab_tests_text, "func testPencilDabRadiusRespondsToLineWidth()", "Pencil dab radius responds to configured size (T111 regression test)"))
    checks.append(require_text(dab_tests_text, "func testCrayonDabRadiusRespondsToLineWidth()", "Crayon dab radius responds to configured size (T111 regression test)"))
    ...
    checks.append(require_text(editor_ui_factory_text, "func floatingPanel(cornerRadius:", "Floating panel creation lives in KCEditorUIFactory (T109 tiered radius)"))
    ...
    checks.append(require_text(main_text, "let paletteButton = self.iconButtonWithSymbolName(\"paintpalette.fill\"", "Top-left palette entry uses palette semantics in code (T110)"))
    checks.append(require_text(main_text, "identifier: \"top.palette\", toControl: paletteButton", "Top-left palette entry keeps palette accessibility wiring (T110)"))
    checks.append(forbid_text(main_text, "brandButton", "Top-left palette entry is not named as a brand/logo button (T110)"))
    ...
    checks.append(require_text(editor_ui_factory_text, "func applyGlassSurface(", "Glass surface chrome is centralized for all glass surfaces (T109)"))
    checks.append(require_text(editor_ui_factory_text, "glassContentTint", "Glass content tint token is centralized (T109)"))
    checks.append(require_text(editor_ui_factory_text, "glassHighlightBorderColor", "Glass white-highlight border token is centralized (T109)"))
    checks.append(require_text(editor_ui_factory_text, "glassShadowColor", "Glass warm-brown shadow color token is centralized (T109)"))
    checks.append(forbid_text(editor_ui_factory_text, "alpha: 0.08).cgColor", "Glass blur no longer uses the old dark hairline border (T109)"))
    checks.append(require_text(editor_ui_factory_text, "func makeGlassEffectView(", "Liquid-glass effect factory is centralized (T109)"))
    checks.append(require_text(editor_ui_factory_text, "UIGlassEffect(style: .regular)", "Liquid glass uses iOS 26 UIGlassEffect where available (T109)"))
    checks.append(require_text(editor_ui_factory_text, "#available(iOS 26.0, *)", "Liquid glass degrades gracefully below iOS 26 (T109)"))
    # T109 G3a：raised 按钮底色 0.92→0.82；border 改玻璃白高光；加顶部 1pt 白色内高光。
    checks.append(require_text(editor_ui_factory_text, "UIColor(white: 1.0, alpha: 0.82)", "Raised button background lowered from 0.92 to 0.82 for glass translucency (T109 G3a)"))
    checks.append(forbid_text(editor_ui_factory_text, "UIColor(white: 1.0, alpha: 0.92)", "Old raised 0.92 opaque background removed in favor of 0.82 glass (T109 G3a)"))
    checks.append(require_text(editor_ui_factory_text, "layerMinXMinYCorner, .layerMaxXMinYCorner", "Raised button has top-only rounded inner highlight (T109 G3a)"))
    checks.append(require_text(editor_ui_factory_text, "heightAnchor.constraint(equalToConstant: 1.0)", "Raised button top inner highlight is 1pt tall (T109 G3a)"))
    # T109 G3b：折叠按钮 + 工具状态 chip 由"实色 0.88"改为真玻璃（makeGlassEffectView）。
    checks.append(require_text(editor_ui_factory_text, "T109 G3b：折叠按钮由", "Collapse toggle button is converted to real glass (T109 G3b)"))
    checks.append(require_text(editor_ui_factory_text, "T109 G3b：工具状态 chip 由", "Tool state chip is converted to real glass (T109 G3b)"))
    # G3a 暗线描边已替换为玻璃白高光；G3b 折叠/chip 不再走 compactBackgroundColor 实色。
    checks.append(forbid_text(editor_ui_factory_text, "view.layer.borderColor = subtleBorderColor\n    }", "Raised buttons use the glass white-highlight border, not subtleBorder (T109 G3a)"))
    ...
    checks.append(require_text(editor_ui_factory_text, "private static let symbolImageCache: NSCache<NSString, UIImage>", "Editor UI factory caches configured SF Symbol images"))
    checks.append(require_text(editor_ui_factory_text, "Self.symbolImageCache.object(forKey:", "Editor UI factory reuses cached configured SF Symbol images"))
    checks.append(require_text(editor_ui_factory_text, "Self.symbolImageCache.setObject", "Editor UI factory stores configured SF Symbol images in cache"))
    ...
    checks.append(require_text(brush_sticker_panel_text, "private static let categorySymbolImageCache: NSCache<NSString, UIImage>", "Brush/stamp panel caches category SF Symbol images"))
    ...
    checks.append(forbid_text(brush_sticker_panel_text, "[8.0, 14.0, 20.0, 28.0]", "Size controls no longer show unsynchronized dot shortcuts (T114)"))
    checks.append(forbid_text(brush_sticker_panel_text, "let dots = UIStackView()", "Size controls keep only the slider and live preview, not static dot shortcuts (T114)"))
    ...
    checks.append(require_text(main_text, "self.editorUIFactory.floatingPanel(cornerRadius:", "Floating panel helper delegates to KCEditorUIFactory with tiered radius (T109)"))
    ...
    checks.append(require_text(toast_presenter_text, "KCEditorVisualStyle.makeGlassEffectView", "Toast uses the liquid-glass effect factory (T109)"))
    checks.append(require_text(toast_presenter_text, "KCEditorVisualStyle.applyGlassSurface", "Toast routes through the unified glass surface chrome (T109)"))
    checks.append(require_text(line_art_picker_text, "KCEditorVisualStyle.applyGlassSurface", "Line-art picker routes through the unified glass surface chrome (T109)"))
    ...
    checks.append(require_text(main_text, "afterDeleteHistoryCount", "Runtime save-history probe verifies saved history delete updates the in-memory list"))
    checks.append(require_text(main_text, "afterDeleteVisible", "Runtime save-history probe verifies deleting the open saved item clears the canvas"))
    ...
    checks.append(require_text(main_text, "configuredPhotoLibraryPickerConfiguration", "Photo picker configuration is centralized for PHPicker"))
    ...
    checks.append(require_text(main_text, "photoPickerPresented", "Runtime system-ui probe verifies photo picker is presented"))
    checks.append(require_text(main_text, "photoPickerDelegateSet", "Runtime system-ui probe verifies photo picker delegate is set"))
    checks.append(require_text(main_text, "photoPickerSelectionLimit", "Runtime system-ui probe verifies photo picker selection limit"))
    ...
    checks.append(require_text(main_text, "runtimeAcceptanceImageImportCompletion", "Runtime system-ui probe waits for async photo import completion"))
    checks.append(require_regex(main_text, r"func finishImportingImage[\s\S]*runtimeAcceptanceImageImportCompletion[\s\S]*runtimeCompletion\?\(\)", "Photo import completion notifies the Debug system-ui probe after async normalization"))
    ...
    checks.append(forbid_regex(view_did_load_text, r"self\.selectToolMode\(\.brush\)[\s\S]{0,120}self\.selectBrushStyle\(\.pencil\)", "Startup does not refresh the default pencil Dock selection twice"))
    checks.append(require_regex(did_tap_brush_button_text, r"self\.canvasView\.currentBrushStyle = button\.brushStyle[\s\S]*self\.selectToolMode\(\.brush\)", "Brush style button applies the new style before the single brush tool refresh"))
    checks.append(forbid_text(did_tap_brush_button_text, "self.selectBrushStyle(button.brushStyle)", "Brush style button does not trigger a second Dock refresh"))
    ...
    checks.append(require_regex(main_text, r"func buildLeftRail[\s\S]*toolScrollView\.showsVerticalScrollIndicator = false[\s\S]*toolScrollView\.alwaysBounceVertical = true", "Left tool rail stays scrollable but hides the visible scrollbar/sliderbar (T112)"))
    checks.append(require_regex(main_text, r"func buildLeftRail[\s\S]*stack\.spacing = self\.leftRailStackSpacing\(\)", "Left rail stack spacing is delegated to device layout metrics"))
    ...
    checks.append(require_regex(editor_ui_factory_text, r"func railToolButton\(symbolName: String, slim: Bool, size: CGFloat, iconPointSize: CGFloat\)[\s\S]*button\.widthAnchor\.constraint\(equalToConstant: size\)[\s\S]*button\.heightAnchor\.constraint\(equalToConstant: size\)", "Left tool buttons use device-driven stable dimensions"))
    ...
    checks.append(require_regex(device_layout_metrics_text, r"var leftRailButtonSize: CGFloat[\s\S]*48\.0 : 56\.0", "Compact iPhone left rail uses smaller buttons so all tools are easier to reach"))
    checks.append(require_regex(device_layout_metrics_text, r"var leftRailStackSpacing: CGFloat[\s\S]*8\.0 : 10\.0", "Compact iPhone left rail uses tighter spacing without changing iPad spacing"))
    checks.append(require_text(main_text, "func leftRailIconPointSize() -> CGFloat", "Left rail icon size follows button metrics"))
    checks.append(require_regex(device_layout_metrics_text, r"var bottomDockWidth: CGFloat[\s\S]*344\.0 : 444\.0", "Bottom brush dock fits the three subtype buttons without excess blank space"))
    checks.append(require_regex(device_layout_metrics_text, r"var bottomDockHeight: CGFloat[\s\S]*66\.0 : 88\.0", "Bottom brush dock uses compact height without crowding iPad"))
    checks.append(require_regex(device_layout_metrics_text, r"var brushCardWidth: CGFloat[\s\S]*96\.0 : 124\.0", "Bottom brush cards fit the tightened dock"))
    checks.append(require_regex(device_layout_metrics_text, r"var brushCardHeight: CGFloat[\s\S]*50\.0 : 62\.0", "Bottom brush cards stay inside the tightened dock"))
    checks.append(forbid_text(main_text, "label.text = KCL10n.brushesPanelTitle", "Bottom dock no longer repeats the current tool category title (T115)"))
    checks.append(require_regex(main_text, r"func buildBottomDock[\s\S]*scrollView\.leadingAnchor\.constraint\(equalTo: panel\.leadingAnchor, constant: self\.bottomDockHorizontalInset\(\)\)", "Bottom dock content starts at the panel edge after removing the category title (T115)"))
    ...
    checks.append(require_text(main_text, "installRightPanelFadeMask(on: rightScrollView)", "Right menu uses a soft fade mask instead of abrupt scroll clipping"))
    ...
    checks.append(require_text(content_picker_feature_text, "private var recentColorSaveWorkItem", "Recent colors debounce UserDefaults persistence"))
    checks.append(require_regex(content_picker_feature_text, r"func addRecentColor[\s\S]*scheduleRecentColorSave\(\)", "Recent color selection schedules debounced persistence"))
    checks.append(forbid_regex(content_picker_feature_text, r"func addRecentColor\(_ color: UIColor\?\) \{[\s\S]{0,320}persistRecentColors\(\)", "Recent color selection does not write UserDefaults synchronously"))
    checks.append(require_text(content_picker_feature_text, "func flushRecentColorSave()", "Recent color persistence can be flushed on lifecycle events"))
    checks.append(require_regex(main_text, r"sceneWillResignActiveNotification[\s\S]*self\.contentPicker\.flushRecentColorSave\(\)[\s\S]*self\.saveDraftIfNeeded\(\)", "Scene deactivation flushes recent colors before draft save"))
    ...
    checks.append(require_text(stroke_render_math_text, "public struct RenderProfile", "Brush visual profile is modeled in KCDrawingEngine"))
    checks.append(require_text(stroke_render_math_text, "public struct TextureLayer", "Brush texture layers are testable engine data"))
    checks.append(require_text(stroke_render_math_text, "dashPatternMultipliers", "Brush texture layers can encode broken wax strokes"))
    checks.append(require_text(stroke_render_math_text, "public static func renderProfile", "Brush style returns a complete render profile"))
    checks.append(require_text(drawing_bridge_text, "func brushRenderProfile(", "Drawing engine protocol exposes brush render profiles"))
    checks.append(require_text(canvas_text, "self.drawingEngine.brushRenderProfile", "Drawing canvas consumes the injected brush profile"))
    checks.append(require_text(canvas_text, "drawTextureLayers(", "Drawing canvas renders profile texture layers"))
    checks.append(require_text(canvas_text, "texturePath.setLineDash", "Drawing canvas applies broken wax dash patterns"))
    checks.append(forbid_text(canvas_text, "textureLayers.filter", "Drawing canvas does not allocate filtered texture-layer arrays in the hot render path"))
    checks.append(require_regex(canvas_text, r"if brushProfile\.grainAlpha > 0\.0 \{[\s\S]*drawCrayonGrain\(", "Crayon brush adds clipped grain texture through profile"))
    ...
    checks.append(require_text(stroke_render_math_text, "lineWidth * 0.32 * pressure", "Pencil render metrics are visibly lighter than pen"))
    checks.append(require_text(stroke_render_math_text, "lineWidth * 0.92", "Pen render metrics keep a stable solid stroke"))
    checks.append(require_text(stroke_render_math_text, "lineWidth * 1.44 * pressure", "Crayon render metrics stay broad without relying only on width"))
    checks.append(require_text(stroke_render_math_text, "min(0.30, 0.14 + pressure * 0.12)", "Pencil base alpha stays below solid ink"))
    checks.append(require_text(stroke_render_math_text, "min(0.038, 0.010 + pressure * 0.028)", "Crayon base alpha stays behind chunky wax texture"))
    ...
    checks.append(require_text(stroke_render_math_text, "widthMultiplier: 2.18", "Crayon profile includes a wide wax smear layer"))
    checks.append(require_text(stroke_render_math_text, "grainAlpha: 1.0", "Crayon grain alpha stays visibly textured"))
    checks.append(require_text(stroke_render_math_tests_text, "testCrayonDefaultStrokeUsesObviousWaxInsteadOfTintedWideLine", "Brush tests guard default crayon against reading as a tinted wide line"))
    checks.append(require_text(stroke_render_math_tests_text, "testCrayonDefaultStrokeHasRoughEdgeAndPaperTooth", "Brush tests guard crayon rough edge and paper tooth"))
    ...
    checks.append(require_text(stroke_render_math_tests_text, "testCrayonWideWaxLayersLeaveVisiblePaperGaps", "Brush tests guard crayon wax layers from becoming smooth marker strokes"))
    checks.append(require_text(stroke_render_math_tests_text, "testCrayonBaseAlphaStaysFarBehindPencil", "Brush tests guard crayon base color from dominating texture"))
    ...
    checks.append(require_text(crayon_grain_text, "let spacing = max(3.4, lineWidth * 0.25)", "Crayon grain leaves visible paper-tooth gaps instead of filling like marker ink"))
    checks.append(require_text(crayon_grain_text, "lineWidth * 0.28", "Crayon grain scales visibly on wide strokes"))
    checks.append(require_text(drawing_bridge_text, "max(1.6, lineWidth * 0.28)", "Crayon grain dash width used by UIKit matches the engine grain model"))
    checks.append(require_text(canvas_text, "layer.kind == .waxSmear ? .butt : path.lineCapStyle", "Crayon wax texture uses blunt broken edges instead of smooth marker caps"))
    ...
    checks.append(require_text(canvas_text, "private let floodFillQueue = DispatchQueue(label: \"com.kidcanvas.canvas.flood-fill\", qos: .userInitiated)", "Flood fill pixel work uses a dedicated serial background queue"))
    checks.append(require_text(canvas_text, "private var floodFillGeneration: Int = 0", "Flood fill protects against stale async results"))
    ...
    checks.append(require_regex(canvas_text, r"func pixelImageExcludingStickers\(at point: CGPoint\)[\s\S]*for stroke in strokes \{[\s\S]*if strokeRenderBounds\(stroke\)\.contains\(point\) \{[\s\S]*drawStroke\(stroke\)", "Single-pixel non-sticker sampling draws only strokes covering the sampled point"))
    checks.append(require_regex(canvas_text, r"func layoutSubviews\(\)[\s\S]*nonStickerRasterCacheImage != nil[\s\S]*nonStickerRasterCacheBounds\.equalTo\(bounds\)[\s\S]*invalidateNonStickerRasterCache\(\)", "Flood fill raster cache invalidates when canvas bounds change"))
    checks.append(require_regex(canvas_text, r"func touchesEnded[\s\S]*strokes\.append\(activeStroke\)[\s\S]*appendCommittedStrokeToRasterCache\(activeStroke\)", "Committing a stroke composes it into the shared non-sticker raster cache"))
    checks.append(require_regex(canvas_text, r"func snapshotImage\(\) -> UIImage[\s\S]*rasterImageExcludingStickers\(includeActiveStroke: activeStroke != nil\)[\s\S]*drawStickerViewsForSnapshot", "Snapshot saving reuses the non-sticker raster and composites stickers instead of rendering the whole canvas layer"))
    checks.append(require_regex(canvas_text, r"func rasterImageExcludingStickers\(includeActiveStroke: Bool = false\) -> UIImage[\s\S]*let cachedImage = nonStickerRasterCacheImage[\s\S]*nonStickerRasterCacheBounds\.equalTo\(bounds\)[\s\S]*nonStickerRasterCacheScale[\s\S]*completedImage = cachedImage[\s\S]*cacheNonStickerRasterImage\(completedImage, scale: scale\)[\s\S]*guard includeActiveStroke, let activeStroke[\s\S]*completedImage\.draw\(in: bounds\)[\s\S]*drawStroke\(activeStroke\)", "Flood fill, snapshot, and active-stroke composition reuse the bounds/scale-matched completed raster"))
    checks.append(require_regex(canvas_text, r"func beginFloodFill[\s\S]*fillColorAlreadyMatchesCanvas\(at: point, fillColor: fillColor\)[\s\S]*return[\s\S]*let previousState = canvasStateSnapshot\(\)[\s\S]*let baseImage = rasterImageExcludingStickers\(\)", "Flood fill skips undo snapshots and full rasterization when the target pixel already has the fill color"))
    checks.append(require_regex(canvas_text, r"func beginFloodFill[\s\S]*floodFillQueue\.async[\s\S]*self\.drawingEngine\.floodFillImage", "Flood fill heavy pixel work runs off the main thread"))
    checks.append(require_text(canvas_text, "DispatchQueue.main.async", "Flood fill switches back to the main thread before applying results"))
    checks.append(require_text(canvas_text, "self?.applyFloodFillResult", "Flood fill results are applied through the guarded result path"))
    checks.append(require_regex(canvas_text, r"func applyFloodFillResult[\s\S]*let resultImage = UIImage\(cgImage: filledImage[\s\S]*backgroundImage = resultImage[\s\S]*strokes\.removeAll\(\)[\s\S]*cacheNonStickerRasterImage\(resultImage, scale: imageScale\)", "Flood fill result becomes the next non-sticker raster cache"))
    checks.append(require_regex(canvas_text, r"func resetCanvasContents\(\)[\s\S]*cancelPendingFloodFillResult\(\)", "Canvas resets invalidate pending asynchronous flood-fill results"))
    checks.append(require_regex(canvas_text, r"func resetCanvasContents\(\)[\s\S]*invalidateNonStickerRasterCache\(\)", "Canvas resets invalidate the flood-fill raster cache"))
    ...
    checks.append(require_text(line_art_feature_text, "private let thumbnailCache", "KCLineArtFeature owns a bounded thumbnail cache"))
    checks.append(require_text(line_art_feature_text, "private let thumbnailRenderingQueue", "Line-art thumbnails can render off the main thread"))
    checks.append(require_text(line_art_feature_text, "thumbnailCache.object(forKey:", "Line-art thumbnails are served from cache before rendering"))
    checks.append(require_text(line_art_feature_text, "thumbnailCache.setObject", "Line-art thumbnails are cached after rendering"))
    checks.append(require_regex(line_art_feature_text, r"func cachedThumbnailImage\(for item: KCLineArtItem\) -> UIImage\?[\s\S]*thumbnailCache\.object\(forKey: item\.id as NSString\)", "Line-art feature exposes cache-only thumbnail lookup"))
    checks.append(require_regex(line_art_feature_text, r"func prepareThumbnailImage\(for item: KCLineArtItem, completion: @escaping \(KCLineArtItem, UIImage\) -> Void\)[\s\S]*thumbnailRenderingQueue\.async[\s\S]*thumbnailImage\(for: item\)[\s\S]*DispatchQueue\.main\.async", "Line-art thumbnail misses render off the main thread and complete on the main thread"))
    checks.append(require_text(line_art_feature_text, "private func renderThumbnailImage(for item: KCLineArtItem) -> UIImage", "Line-art thumbnail rendering is separated from cache lookup"))
    checks.append(require_text(line_art_feature_text, "func lineArtImage(for item: KCLineArtItem, canvasSize: CGSize, drawingRect: CGRect? = nil) -> UIImage", "KCLineArtFeature renders canvas-sized line-art images"))
    checks.append(require_text(line_art_feature_text, "drawingRect ?? CGRect(origin: .zero, size: canvasSize).insetBy(dx: 110.0, dy: 90.0)", "Line-art rendering keeps the legacy full-canvas fallback"))
    ...
    checks.append(forbid_regex(line_art_picker_text, r"func lineArtPreviewButton\(for item: KCLineArtItem, index: Int\) -> UIButton[\s\S]*thumbnailImage\(for: item\)", "Line-art picker does not synchronously render thumbnails while building buttons"))
    ...
    checks.append(require_text(main_text, "func currentLineArtItems() -> [KCLineArtItem]", "Main view controller lazily loads line-art items only when needed"))
    checks.append(require_regex(main_text, r"func currentLineArtItems\(\) -> \[KCLineArtItem\][\s\S]*self\.lineArtFeature\.makeLineArtItems\(\)", "Main view controller delegates line-art item creation to KCLineArtFeature only through the lazy loader"))
    checks.append(require_text(main_text, "items: self.currentLineArtItems()", "Line-art picker receives lazily loaded line-art items"))
    checks.append(require_text(main_text, "let lineArtItem = self.currentLineArtItems().first", "Runtime drawing-tools probe reuses the lazy line-art item loader"))
    ...
    checks.append(require_regex(main_text, r"selectionHandler:[\s\S]*performCanvasReplacementAfterUserConfirmation[\s\S]*loadLineArtItem\(item\)", "Line-art picker selection protects the current canvas before loading the item"))
    checks.append(require_text(main_text, "func visibleLineArtDrawingRect(forCanvasSize canvasSize: CGSize) -> CGRect", "Main view controller computes the visible line-art drawing rect"))
    checks.append(require_text(main_text, "self.lineArtFeature.lineArtImage(for: item, canvasSize: canvasSize, drawingRect: drawingRect)", "Main view controller delegates visible-area line-art canvas rendering to KCLineArtFeature"))
    checks.append(require_regex(main_text, r"func loadLineArtItem\(_ item: KCLineArtItem[\s\S]*var canvasSize = self\.canvasView\.bounds\.size[\s\S]*if canvasSize == \.zero \{[\s\S]*self\.view\.layoutIfNeeded\(\)[\s\S]*self\.canvasView\.layoutIfNeeded\(\)[\s\S]*canvasSize = self\.canvasView\.bounds\.size", "Line-art loading only falls back to synchronous layout when canvas bounds are zero"))
    checks.append(require_text(main_text, "private let lineArtRenderingQueue", "Line-art canvas rendering has a dedicated background queue"))
    checks.append(require_text(main_text, "private var lineArtLoadGeneration", "Line-art async rendering uses a generation guard"))
    checks.append(require_regex(main_text, r"lineArtRenderingQueue\.async[\s\S]*lineArtFeature\.lineArtImage\(for: item, canvasSize: canvasSize, drawingRect: drawingRect\)[\s\S]*DispatchQueue\.main\.async", "Line-art canvas image renders off the main thread and applies on the main thread"))
    checks.append(require_text(main_text, "func invalidateLineArtLoadWork()", "Line-art async rendering can be invalidated when canvas content changes"))
    checks.append(require_text(main_text, "lineArtLoaded", "Runtime drawing-tools probe verifies async line-art loading completed"))
    ...
    checks.append(require_regex(main_text, r"func didTapStickerCategoryButton[\s\S]*self\.contentPicker\.selectStickerCategory\(category\)[\s\S]*reloadStickerButtons[\s\S]*self\.selectToolMode\(\.sticker\)", "Sticker category buttons switch the visible sticker set and enter one-shot stamp mode"))
    ...
    checks.append(require_text(main_text, "static let stickerButtons: TimeInterval = 1.10", "Sticker buttons load after critical startup history/draft work"))
    checks.append(require_regex(main_text, r"func buildLeftRail[\s\S]*stack\.axis = \.vertical[\s\S]*stack\.alignment = \.center", "Left tool rail centers fixed-width tool buttons instead of stretching them across the scroll frame"))
    checks.append(require_text(main_text, "stack.widthAnchor.constraint(greaterThanOrEqualTo: toolScrollView.frameLayoutGuide.widthAnchor)", "Left tool rail scroll content can exceed the frame width without forcing tool buttons wider"))
    checks.append(forbid_text(main_text, "stack.widthAnchor.constraint(equalTo: toolScrollView.frameLayoutGuide.widthAnchor)", "Left tool rail avoids startup Auto Layout conflicts from forcing 48pt buttons into a 56pt stack width"))
    checks.append(require_regex(main_text, r"func buildBottomDock[\s\S]*stack\.axis = \.horizontal[\s\S]*stack\.alignment = \.center", "Bottom brush dock centers fixed-height brush cards instead of stretching them across the scroll frame"))
    checks.append(require_text(main_text, "stack.heightAnchor.constraint(greaterThanOrEqualTo: scrollView.frameLayoutGuide.heightAnchor)", "Bottom brush dock scroll content can exceed the frame height without forcing brush cards taller"))
    checks.append(forbid_text(main_text, "stack.heightAnchor.constraint(equalTo: scrollView.frameLayoutGuide.heightAnchor)", "Bottom brush dock avoids startup Auto Layout conflicts from forcing 44pt cards into a 48pt stack height"))
    checks.append(require_text(brush_sticker_panel_text, "sizeSlider.addTarget", "Size controls keep the slider as the primary size input (T114)"))
    checks.append(require_text(brush_sticker_panel_text, "sizePreviewView.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 10.0)", "Size controls keep a compact inline live preview without unsynchronized dot shortcuts (T114)"))
    checks.append(forbid_text(brush_sticker_panel_text, "shell.backgroundColor = UIColor(white: 1.0, alpha: 0.58)", "Size controls no longer stack an extra translucent shell over the glass panel (T109 G4)"))
    checks.append(require_text(brush_sticker_panel_text, "stickerRow.heightAnchor.constraint(greaterThanOrEqualTo: stickerScrollView.frameLayoutGuide.heightAnchor)", "Sticker scroll content can exceed the frame height without forcing stamp buttons taller"))
    checks.append(forbid_text(brush_sticker_panel_text, "stickerRow.heightAnchor.constraint(equalTo: stickerScrollView.frameLayoutGuide.heightAnchor)", "Sticker row avoids startup Auto Layout conflicts from forcing 44pt stamps into a 48pt stack height"))
    checks.append(require_text(brush_sticker_panel_text, "button.setContentCompressionResistancePriority(.defaultLow, for: .vertical)", "Sticker buttons can yield vertically to the scroll row during startup layout"))
    checks.append(forbid_text(main_text, "after: 0.36", "Startup draft restore is not artificially delayed to 0.36s"))
    checks.append(forbid_text(main_text, "after: 0.62", "Startup sticker loading is not artificially delayed to 0.62s"))
    ...
    checks.append(require_regex(main_text, r"func processImportedImage\([\s\S]*self\.imageImportProcessingQueue\.async[\s\S]*normalizedImageFromImage\(image\)[\s\S]*DispatchQueue\.main\.async", "Album import normalizes large photos off the main thread"))
    ...
    checks.append(require_text(session_store_bridge_text, "private var draftImageCache", "Session service caches decoded draft images"))
    checks.append(require_text(session_store_bridge_text, "private var draftThumbnailCache", "Session service caches decoded draft thumbnail images"))
    checks.append(require_regex(session_store_bridge_text, r"func loadDraftImage\(\)[\s\S]*if let draftImageCache[\s\S]*return draftImageCache[\s\S]*cacheLoadedDraftImage\(image\)", "Draft image lookup uses cache before decoding from disk"))
    checks.append(require_regex(session_store_bridge_text, r"func displayDecodedImage\(from data: Data\) -> UIImage\?[\s\S]*UIImage\(data: data\)[\s\S]*Self\.displayDecodedImage\(image\)", "Session service exposes a display-decoding image loader"))
    checks.append(require_regex(session_store_bridge_text, r"private static func displayDecodedImage\(_ image: UIImage\) -> UIImage\?[\s\S]*UIGraphicsImageRenderer\(size: imageSize, format: format\)\.image[\s\S]*image\.draw\(in:", "Display-decoding image loader forces bitmap decode through UIGraphicsImageRenderer"))
    checks.append(require_regex(session_store_bridge_text, r"func cachedDraftThumbnailImage\(\) -> UIImage\?[\s\S]*draftCacheLock\.lock\(\)[\s\S]*let thumbnail = draftThumbnailCache[\s\S]*return thumbnail", "Session service exposes lock-protected cache-only draft thumbnail lookup"))
    checks.append(require_regex(session_store_bridge_text, r"func cacheLoadedDraftImage\(_ image: UIImage\)[\s\S]*let thumbnail = Self\.generateThumbnail\(from: image\)[\s\S]*draftCacheLock\.lock\(\)[\s\S]*draftImageCache = image[\s\S]*draftThumbnailCache = thumbnail", "Session service can cache asynchronously restored drafts"))
    checks.append(require_regex(session_store_bridge_text, r"func artworkData\(forSession session: KCSessionMetadata\) -> Data\?[\s\S]*KCArtworkSession[\s\S]*store\.artworkData\(for: artworkSession\)", "Session service can read artwork data from already loaded metadata without touching the metadata cache"))
    checks.append(require_regex(kc_session_store_text, r"func hasDraft\(\) -> Bool[\s\S]*fileManager\.fileExists\(atPath: draftURL\.path\)", "Draft existence check avoids loading draft image data"))
    checks.append(require_regex(session_store_bridge_text, r"func hasDraft\(\) -> Bool[\s\S]*draftImageCache != nil[\s\S]*store\.hasDraft\(\)", "Session service exposes a lightweight draft existence check"))
    checks.append(require_regex(session_store_bridge_text, r"func draftThumbnailImage\(\) -> UIImage\?[\s\S]*if let draftThumbnailCache[\s\S]*return draftThumbnailCache[\s\S]*Self\.generateThumbnail", "Draft thumbnail lookup uses a separate thumbnail cache"))
    checks.append(require_text(refresh_history_text, "loadDraftThumbnail ? self.sessionStore.draftThumbnailImage() : self.sessionStore.cachedDraftThumbnailImage()", "History refresh can skip synchronous draft thumbnail loading during startup"))
    checks.append(forbid_text(refresh_history_text, "loadDraftImage()", "History refresh does not decode the full draft image for the thumbnail slot"))
    checks.append(require_regex(main_text, r"func didTapDeleteLatestSession\(\)[\s\S]*let hasDraft = self\.sessionStore\.hasDraft\(\)[\s\S]*let shouldDeleteDraft", "Delete flow checks draft existence without decoding the draft image"))
    checks.append(require_text(session_store_bridge_text, "func saveDraftData(pngData: Data, cachedImage: UIImage?) -> Bool", "Draft save can refresh the in-memory draft image cache"))
    checks.append(require_regex(session_store_bridge_text, r"func clearDraft\(\)[\s\S]*draftImageCache = nil[\s\S]*draftThumbnailCache = nil[\s\S]*store\.clearDraft\(\)", "Clearing a draft invalidates draft image caches"))
    checks.append(require_regex(session_store_bridge_text, r"func saveImage[\s\S]*encodedArtworkData\(from: image\)[\s\S]*saveArtwork", "Session save routes UIImage encoding through the shared encoded-artwork helper"))
    checks.append(require_regex(session_store_bridge_text, r"func encodedArtworkData\(from image: UIImage\)[\s\S]*image\.pngData\(\)[\s\S]*jpegData\(compressionQuality: 0\.85\)", "Session service exposes a reusable PNG/JPEG encoding helper"))
    checks.append(require_text(session_store_bridge_text, "thumbnailImageCache", "Session service caches decoded thumbnail images"))
    checks.append(require_text(session_store_bridge_text, "cache.countLimit = 100", "Thumbnail image cache has a bounded item count"))
    checks.append(require_text(session_store_bridge_text, "guard findSession(id: sessionId) != nil else", "Thumbnail cache hit is guarded by live session metadata"))
    checks.append(require_text(session_store_bridge_text, "thumbnailImageCache.object(forKey: sessionId as NSString)", "Thumbnail lookup uses cache before decoding"))
    checks.append(require_regex(session_store_bridge_text, r"func cachedThumbnailImage\(forSessionId sessionId: String\) -> UIImage\?[\s\S]*guard findSession\(id: sessionId\) != nil else[\s\S]*thumbnailImageCache\.object\(forKey: sessionId as NSString\)", "Session service exposes cache-only thumbnail lookup"))
    checks.append(require_regex(session_store_bridge_text, r"func cachedThumbnailImage\(forSession session: KCSessionMetadata\) -> UIImage\?[\s\S]*guard !session\.identifier\.isEmpty[\s\S]*thumbnailImageCache\.object\(forKey: session\.identifier as NSString\)", "Session service exposes metadata-based cache-only thumbnail lookup"))
    checks.append(require_text(session_store_bridge_text, "thumbnailImageCache.setObject(cachedThumbnail, forKey: session.id as NSString)", "Save path refreshes cached thumbnail image"))
    checks.append(require_text(session_store_bridge_text, "thumbnailImageCache.removeObject(forKey: sessionId as NSString)", "Delete path invalidates cached thumbnail image"))
    checks.append(require_text(session_store_bridge_text, "thumbnailPreloadQueue", "History thumbnail preloading has a background queue"))
    checks.append(require_regex(session_store_bridge_text, r"func preloadThumbnailImages\(forSessionIds sessionIds: \[String\], completion: \(\(\) -> Void\)\? = nil\)[\s\S]*thumbnailImageCache\.object[\s\S]*enqueueThumbnailPreloadCompletion\(for: session\.id, completion: waiter\)[\s\S]*thumbnailPreloadQueue\.async", "History thumbnail preloading skips cached ids and coalesces in-flight ids before background decode"))
    checks.append(require_text(session_store_bridge_text, "thumbnailPreloadCompletions", "History thumbnail preloading coalesces callbacks for ids already in flight"))
    checks.append(require_regex(session_store_bridge_text, r"private func enqueueThumbnailPreloadCompletion[\s\S]*thumbnailPreloadCompletions\[sessionId, default: \[\]\]\.append\(completion\)", "History thumbnail preload waiters attach to the existing in-flight id"))
    checks.append(require_regex(session_store_bridge_text, r"private func drainThumbnailPreloadCompletions[\s\S]*thumbnailPreloadCompletions\[sessionId\] = nil", "History thumbnail preload waiters are drained exactly once when the id finishes"))
    checks.append(require_regex(session_store_bridge_text, r"for session in sessionsToPreload[\s\S]*let completions = self\.drainThumbnailPreloadCompletions\(for: session\.id\)[\s\S]*if !completions\.isEmpty \{[\s\S]*DispatchQueue\.main\.async", "History thumbnail preload completion fires after shared in-flight decoding finishes"))
    checks.append(forbid_text(refresh_history_text, "thumbnailImage(forSessionId:", "History refresh does not synchronously decode visible saved thumbnails"))
    checks.append(forbid_text(refresh_history_text, "cachedThumbnailImage(forSessionId:", "History refresh avoids repeated metadata lookup for visible saved thumbnails"))
    checks.append(require_text(refresh_history_text, "cachedThumbnailImage(forSession: session)", "History refresh uses already-loaded metadata for cache-only saved thumbnails"))
    checks.append(require_text(refresh_history_text, "missingVisibleThumbnailIds.append(session.identifier)", "History refresh records visible thumbnail cache misses"))
    checks.append(require_regex(refresh_history_text, r"else \{[\s\S]*cachedThumbnailImage\(forSession: session\)[\s\S]*Self\.setHistoryButtonPlaceholderVisible\(false, on: button\)", "Saved history thumbnails do not show the default photo placeholder while loading"))
    checks.append(forbid_text(editor_ui_factory_text, "button.setImage(Self.historySlotPlaceholderImage(), for: state)", "History thumb factory does not install default placeholders that can leak into selected/highlighted states"))
    checks.append(forbid_text(editor_ui_factory_text, "adjustsImageWhenHighlighted", "History thumb buttons do not use deprecated highlighted-image adjustment"))
    checks.append(forbid_text(editor_ui_factory_text, "adjustsImageWhenDisabled", "History thumb buttons do not use deprecated disabled-image adjustment"))
    checks.append(require_regex(main_text, r"func preloadVisibleHistoryThumbnailsIfNeeded\(_ sessionIds: \[String\]\)[\s\S]*historyThumbnailRefreshGeneration[\s\S]*preloadThumbnailImages\(forSessionIds: uniqueSessionIds\) \{ \[weak self\][\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, preloadThumbnails: false, loadSessions: false\)", "Visible history thumbnail misses refresh from cache without reloading draft thumbnails or sessions"))
    checks.append(require_regex(main_text, r"func refreshHistoryUI\([^\)]*\)[\s\S]*self\.preloadVisibleHistoryThumbnailsIfNeeded\(missingVisibleThumbnailIds\)[\s\S]*self\.preloadAdjacentHistoryThumbnails\(\)", "History refresh schedules adjacent thumbnail preloading after visible slots"))
    checks.append(require_regex(main_text, r"func preloadAdjacentHistoryThumbnails\(\)[\s\S]*KCHistoryPaging\([\s\S]*adjacentPageSessionIndexes\(\)[\s\S]*preloadThumbnailImages\(forSessionIds:", "Controller delegates adjacent thumbnail preload indexes to KCHistoryPaging"))
    checks.append(require_regex(main_text, r"func didTapPreviousHistoryPage\(\)[\s\S]*self\.historyPageIndex -= 1[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)", "Previous history page refreshes from memory without rereading draft thumbnail or sessions"))
    checks.append(require_regex(main_text, r"func didTapNextHistoryPage\(\)[\s\S]*self\.historyPageIndex \+= 1[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)", "Next history page refreshes from memory without rereading draft thumbnail or sessions"))
    checks.append(require_text(session_store_bridge_text, "artworkSessionCache", "Session service caches decoded session metadata"))
    checks.append(require_text(session_store_bridge_text, "private let sessionMetadataQueue", "Session metadata can load off the main thread"))
    checks.append(require_text(session_store_bridge_text, "private let artworkSessionCacheLock", "Session metadata cache is protected by a lock"))
    checks.append(require_regex(session_store_bridge_text, r"func loadAllSessionsAsync\(completion: @escaping \(\[KCSessionMetadata\]\) -> Void\)[\s\S]*sessionMetadataQueue\.async[\s\S]*loadAllSessions\(\)[\s\S]*DispatchQueue\.main\.async", "Session metadata async loader reads off the main thread and completes on the main thread"))
    checks.append(require_text(session_store_bridge_text, "cachedArtworkSessions()", "Session queries share a cached metadata loader"))
    checks.append(require_text(session_store_bridge_text, "artworkSessionCache = sessions", "Session metadata loader stores successful disk reads"))
    checks.append(require_regex(session_store_bridge_text, r"func cachedArtworkSessions\(\) -> \[KCArtworkSession\][\s\S]*artworkSessionCacheLock\.lock\(\)[\s\S]*store\.loadSessions\(\)[\s\S]*artworkSessionCache = sessions", "Session metadata cache lookup is guarded and fills from disk only on miss"))
    checks.append(require_regex(session_store_bridge_text, r"func saveArtwork\([\s\S]*existingSessionId\.flatMap \{ findSession\(id: \$0\) \}", "Session save resolves existing sessions through the metadata cache"))
    checks.append(forbid_regex(session_store_bridge_text, r"func saveArtwork\([\s\S]*store\.loadSessions\(\)\)\?\.first", "Session save does not reread metadata from disk when resolving an existing session"))
    checks.append(require_text(session_store_bridge_text, "replaceCachedSession(session)", "Save path refreshes cached session metadata"))
    checks.append(require_text(session_store_bridge_text, "removeCachedSession(id: sessionId)", "Delete path invalidates cached session metadata"))
    ...
    checks.append(require_regex(kc_session_store_text, r"rollbackBackup\(for: artworkURL[\s\S]*rollbackBackup\(for: thumbnailURL[\s\S]*restore\(artworkBackup\)[\s\S]*restore\(thumbnailBackup\)", "Failed session saves restore previous artwork files through file backups"))
    checks.append(require_text(kc_session_store_text, "避免为了回滚把旧大图整份读入内存", "Session save rollback documents file-level backup instead of old artwork Data reads"))
    checks.append(require_text(kc_session_store_text, "replaceItemAt", "Session save rollback restores backup files without deleting the destination first"))
    checks.append(require_text(kc_session_store_text, "恢复失败时保留 .rollback 文件", "Session save rollback keeps backup files if restore fails"))
    checks.append(forbid_regex(kc_session_store_text, r"previousArtwork|previousThumbnail|previousData", "Session save rollback no longer stores previous artwork bytes in memory"))
    checks.append(require_regex(kc_session_store_text, r"func delete\(_ session: KCArtworkSession\) throws[\s\S]*try writeMetadataDocument[\s\S]*try\? fileManager\.removeItem", "Session delete writes metadata before removing artwork files"))
    ...
    checks.append(require_text(drawing_bridge_text, "lineWidth: Double", "brushDabs carries user lineWidth to scale dab radius (T111)"))
    checks.append(require_text(drawing_bridge_text, ".scaledForLineWidth(lineWidth)", "brushDabs applies scaledForLineWidth to the preset so pencil/crayon size takes effect (T111)"))
    checks.append(require_text(canvas_text, "drawingEngine.brushDabs(", "Canvas generates dabs via the engine bridge"))
    checks.append(require_text(canvas_text, "lineWidth: Double(stroke.lineWidth)", "Canvas resolves dabs with stroke.lineWidth so pencil/crayon size takes effect (T111)"))
    ...
    checks.append(require_text(canvas_text, "stroke.cachedRenderBounds = renderBounds", "Canvas stores computed stroke render bounds"))
    checks.append(require_regex(canvas_text, r"func cachedCrayonGrainDashPoints\(forStroke stroke: KDStroke[\s\S]*if let cachedCrayonGrainDashPoints = stroke\.cachedCrayonGrainDashPoints[\s\S]*return cachedCrayonGrainDashPoints[\s\S]*stroke\.cachedCrayonGrainDashPoints = dashPoints", "Canvas reuses cached crayon grain dash points per stroke"))
    checks.append(require_regex(canvas_text, r"func drawCrayonGrain[\s\S]*let grainPath = crayonGrainPath\([\s\S]*grainPath\.stroke\(\)", "Canvas batches crayon grain dashes into a single path stroke"))
    checks.append(require_regex(canvas_text, r"func drawCrayonGrain[\s\S]*let paperToothPath = crayonGrainPath\([\s\S]*paperToothPath\.stroke\(\)", "Crayon rendering adds clipped paper-tooth gaps so it does not read as a smooth marker"))
    checks.append(require_regex(canvas_text, r"func drawCrayonGrain[\s\S]*let secondaryToothPath = crayonGrainPath\([\s\S]*secondaryToothPath\.stroke\(\)", "Crayon rendering adds a second paper-tooth pass for rough wax texture"))
    checks.append(require_text(canvas_text, "private func crayonGrainPath(", "Crayon grain path batching is centralized for colored grain and paper-tooth passes"))
    checks.append(forbid_regex(canvas_text, r"func drawCrayonGrain[\s\S]*let dash = UIBezierPath\(\)[\s\S]*dash\.stroke\(\)", "Canvas does not allocate one UIBezierPath per crayon grain dash"))
    checks.append(require_regex(canvas_text, r"func invalidateStrokeRenderBounds\(_ stroke: KDStroke\)[\s\S]*stroke\.cachedRenderBounds = nil[\s\S]*stroke\.cachedCrayonGrainDashPoints = nil", "Canvas invalidates stroke render caches after path mutations"))
    checks.append(require_text(canvas_text, "invalidateStrokeRenderBounds(activeStroke)", "Canvas invalidates active-stroke render bounds after path mutations"))
    checks.append(require_text(canvas_text, "private func strokeRenderBounds(_ stroke: KDStroke) -> CGRect", "Canvas computes conservative stroke redraw bounds"))
    checks.append(require_text(canvas_text, "private func setNeedsDisplayForStrokeBounds(_ redrawBounds: CGRect)", "Canvas has a local dirty-rect invalidation helper"))
    checks.append(require_regex(canvas_text, r"func touchesMoved[\s\S]*let previousStrokeBounds = strokeRenderBounds\(activeStroke\)[\s\S]*previousStrokeBounds\.union\(strokeRenderBounds\(activeStroke\)\)[\s\S]*setNeedsDisplayForStrokeBounds", "Moving touches redraw only the union of old and new active-stroke bounds"))
    checks.append(require_regex(canvas_text, r"func touchesEnded[\s\S]*let previousStrokeBounds = strokeRenderBounds\(activeStroke\)[\s\S]*setNeedsDisplayForStrokeBounds\(previousStrokeBounds\.union\(strokeRenderBounds\(activeStroke\)\)\)", "Ending touches redraw only the committed stroke bounds"))
    ...
    checks.append(require_regex(canvas_text, r"private var canvasHasVisibleContent: Bool \{[\s\S]*backgroundImage != nil \|\| !strokes\.isEmpty \|\| !stickers\.isEmpty[\s\S]*\}", "Visible-content checks avoid allocating a full canvas state snapshot"))
    checks.append(forbid_text(canvas_text, "canvasStateHasVisibleContent(canvasStateSnapshot())", "Visible-content checks must not copy strokes just to test emptiness"))
    checks.append(require_regex(canvas_text, r"func canvasStateSnapshot\(\)[\s\S]*state\.strokes = strokes", "Canvas state snapshots share append-only committed strokes"))
    checks.append(forbid_regex(canvas_text, r"func canvasStateSnapshot\(\)[\s\S]*strokes\.map \{ copyOfStroke\(\$0\) \}", "Canvas state snapshots do not deep-copy every committed stroke"))
    checks.append(require_regex(canvas_text, r"func applyCanvasState\(_ state: KDCanvasState\)[\s\S]*for stroke in state\.strokes[\s\S]*strokes\.append\(copyOfStroke\(stroke\)\)", "Applying a canvas state still deep-copies strokes into the active canvas"))
    ...
    checks.append(require_text(main_text, "private var appliedCanvasActionState: KCCanvasFeature.ActionState?", "Main view controller caches the applied canvas action state"))
    ...
    checks.append(require_regex(main_text, r"func refreshActionButtons[\s\S]*let actionState = self\.canvasFeature\.actionState\(for: self\.canvasView\)[\s\S]*if self\.appliedCanvasActionState == actionState[\s\S]*return[\s\S]*self\.appliedCanvasActionState = actionState", "Action buttons skip repeated UIKit appearance writes when state is unchanged"))
    ...
    checks.append(require_regex(main_text, r"func isDraftProtectionGenerationCurrent\(_ generation: Int\) -> Bool[\s\S]*draftGenerationLock\.lock\(\)[\s\S]*return self\.draftProtectionGeneration == generation", "Draft protection generation can be checked from the background queue"))
    checks.append(require_regex(main_text, r"func preserveUnsavedActiveSessionDraftIfNeeded[\s\S]*self\.activeSession != nil && !self\.activeSessionHasUnsavedChanges[\s\S]*return false[\s\S]*self\.activeDraftMatchesCanvas && self\.sessionStore\.hasDraft\(\)[\s\S]*return true[\s\S]*self\.canvasFeature\.hasVisibleContent\(self\.canvasView\)[\s\S]*let snapshot = self\.canvasView\.snapshotImage\(\)[\s\S]*self\.draftPersistenceQueue\.async[\s\S]*snapshot\.pngData\(\)[\s\S]*self\.sessionStore\.saveDraftData\(pngData: pngData, cachedImage: snapshot\)[\s\S]*return true", "Visible new canvases and dirty saved sessions protect a draft via background PNG encoding and disk save"))
    checks.append(require_regex(main_text, r"func preserveUnsavedActiveSessionDraftIfNeeded\(\) -> Bool[\s\S]*guard let pngData = snapshot\.pngData\(\) else[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)[\s\S]*let saved = self\.sessionStore\.saveDraftData\(pngData: pngData, cachedImage: snapshot\)[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)", "Draft protection callbacks refresh history without rereading draft thumbnails or sessions"))
    checks.append(forbid_regex(main_text, r"func preserveUnsavedActiveSessionDraftIfNeeded[\s\S]*self\.sessionStore\.saveDraftImage\(snapshot\)", "Draft protection no longer encodes/saves PNG synchronously on the main thread"))
    checks.append(forbid_regex(main_text, r"func preserveUnsavedActiveSessionDraftIfNeeded[\s\S]*DispatchQueue\.main\.sync", "Draft protection does not block the background persistence queue on main-sync generation checks"))
    checks.append(require_text(main_text, "func performCanvasReplacementAfterUserConfirmation", "Canvas replacement confirmation entry exists"))
    checks.append(require_text(main_text, "private func protectCurrentCanvasDraftBeforeReplacement", "Canvas replacement protects dirty work before continuing"))
    checks.append(require_text(main_text, "KCL10n.replaceCanvasAlertTitle", "Canvas replacement confirmation title is localized"))
    checks.append(require_text(main_text, "KCL10n.saveDraftAndContinueTitle", "Canvas replacement continue action is localized"))
    checks.append(require_regex(main_text, r"func performCanvasReplacementAfterUserConfirmation[\s\S]*self\.protectCurrentCanvasDraftBeforeReplacement[\s\S]*guard saved else[\s\S]*self\.showSaveToastWithSuccess\(false\)[\s\S]*replacement\(\)", "Canvas replacement proceeds only after draft protection succeeds"))
    checks.append(require_regex(main_text, r"func didTapImportImage\(\)[\s\S]*performCanvasReplacementAfterUserConfirmation[\s\S]*presentPhotoLibraryPicker", "Album import confirms before replacing the current canvas"))
    checks.append(require_regex(main_text, r"func didTapOpenLatestSession\(\)[\s\S]*performCanvasReplacementAfterUserConfirmation[\s\S]*openSession\(session\)", "Open-latest confirms before replacing the current canvas"))
    checks.append(require_regex(main_text, r"func didTapHistoryThumb[\s\S]*performCanvasReplacementAfterUserConfirmation[\s\S]*selectedHistorySession = session[\s\S]*openSession\(session\)", "Saved history thumbs confirm before replacing the current canvas"))
    checks.append(require_regex(main_text, r"func didTapDraftThumb\(\)[\s\S]*performCanvasReplacementAfterUserConfirmation[\s\S]*openDraftThumb\(\)", "Draft thumb confirms before replacing the current canvas"))
    checks.append(require_regex(main_text, r"func clearDraftAndInvalidateCurrentDraftMarker\(\)[\s\S]*self\.sessionStore\.clearDraft\(\)[\s\S]*self\.activeDraftMatchesCanvas = false", "Clearing a draft also invalidates the current-draft marker"))
    checks.append(require_count_exact(main_text, "self.sessionStore.clearDraft()", 1, "Main controller clears drafts only through clearDraftAndInvalidateCurrentDraftMarker"))
    ...
    checks.append(require_regex(main_text, r"func saveDraftIfNeeded[\s\S]*self\.draftPersistenceQueue\.async[\s\S]*snapshot\.pngData\(\)[\s\S]*isDraftSaveGenerationCurrent\(generation\)[\s\S]*self\.sessionStore\.saveDraftData\(pngData: pngData, cachedImage: snapshot\)[\s\S]*DispatchQueue\.main\.async", "Draft autosave encodes and writes PNG off the main thread"))
    checks.append(require_regex(main_text, r"func saveDraftIfNeeded[\s\S]*guard self\.isDraftSaveGenerationCurrent\(generation\) else \{ return \}[\s\S]*let saved = self\.sessionStore\.saveDraftData\(pngData: pngData, cachedImage: snapshot\)[\s\S]*DispatchQueue\.main\.async[\s\S]*guard self\.isDraftSaveGenerationCurrent\(generation\) else \{ return \}[\s\S]*self\.activeDraftMatchesCanvas = saved", "Draft autosave writes only after stale-result guard and marks the draft current only on success"))
    checks.append(require_regex(main_text, r"func saveDraftIfNeeded\(\)[\s\S]*self\.activeSession != nil && !self\.activeSessionHasUnsavedChanges[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)[\s\S]*!self\.canvasFeature\.hasVisibleContent\(self\.canvasView\)[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)[\s\S]*guard let pngData else[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)[\s\S]*self\.activeDraftMatchesCanvas = saved[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)", "Draft autosave callbacks refresh history without rereading draft thumbnails or sessions"))
    checks.append(forbid_regex(main_text, r"DispatchQueue\.main\.async \{ \[weak self\] in\n(?:(?!\s*\}).*\n){0,12}\s*let saved = self\.sessionStore\.saveDraftData\(pngData:", "Draft autosave does not write draft data on the main thread"))
    checks.append(require_regex(main_text, r"func openSession\(_ session: KCSessionMetadata, completion: \(\(Bool\) -> Void\)\? = nil\)[\s\S]*let generation = self\.nextArtworkLoadGeneration\(\)[\s\S]*self\.artworkLoadingQueue\.async[\s\S]*artworkData\(forSession: session\)[\s\S]*displayDecodedImage\(from: data\)[\s\S]*DispatchQueue\.main\.async[\s\S]*guard self\.artworkLoadGeneration == generation[\s\S]*self\.canvasView\.restoreCanvas\(with: image\)", "Opening saved artwork display-decodes the full image off the main thread before restoring"))
    checks.append(forbid_regex(main_text, r"func openSession\(_ session: KCSessionMetadata[\s\S]*artworkImage\(forSessionId:", "Opening saved artwork does not synchronously decode UIImage through the session service"))
    checks.append(forbid_regex(main_text, r"func openSession\(_ session: KCSessionMetadata[\s\S]*artworkData\(forSessionId:", "Opening saved artwork does not touch metadata-cache lookup from the background path"))
    checks.append(require_regex(main_text, r"func didTapDraftThumb\(\)[\s\S]*let generation = self\.nextArtworkLoadGeneration\(\)[\s\S]*self\.draftPersistenceQueue\.async[\s\S]*loadDraftData\(\)[\s\S]*displayDecodedImage\(from: data\)[\s\S]*DispatchQueue\.main\.async[\s\S]*guard self\.artworkLoadGeneration == generation[\s\S]*self\.canvasView\.restoreCanvas\(with: draftImage\)", "Opening a draft display-decodes the full draft off the main thread before restoring"))
    checks.append(forbid_regex(main_text, r"func didTapDraftThumb\(\)[\s\S]*UIImage\(data:", "Opening a draft does not leave UIImage lazy decode in the controller"))
    checks.append(forbid_regex(main_text, r"func didTapDraftThumb\(\)[\s\S]*loadDraftImage\(\)", "Opening a draft no longer synchronously decodes the full draft image"))
    checks.append(require_regex(main_text, r"func drawingCanvasViewContentDidChange[\s\S]*self\.invalidateArtworkLoadWork\(\)", "Canvas edits invalidate stale full-artwork and draft open requests"))
    checks.append(forbid_regex(main_text, r"override func viewDidLoad\(\)[\s\S]*self\.sessionStore\.loadAllSessions\(\)[\s\S]*self\.buildInterface\(\)", "Startup viewDidLoad does not synchronously load session metadata before building the first frame"))
    checks.append(forbid_regex(main_text, r"override func viewDidLoad\(\)[\s\S]*self\.lineArtFeature\.makeLineArtItems\(\)[\s\S]*self\.buildInterface\(\)", "Startup viewDidLoad does not build line-art item metadata before it is needed"))
    checks.append(require_text(main_text, "self.refreshHistoryUI(loadDraftThumbnail: false, preloadThumbnails: false, loadSessions: false, checkDraftExistence: false)", "Initial history refresh skips metadata, thumbnail preloading, and draft file checks before the first frame"))
    checks.append(forbid_regex(view_did_load_text, r"contentPicker\.loadRecentColors\(\)", "Startup viewDidLoad does not synchronously read recent colors"))
    checks.append(forbid_regex(view_did_load_text, r"self\.reloadPaletteGrid\(\)", "Startup viewDidLoad does not build palette color buttons before the first frame"))
    checks.append(require_text(main_text, "func loadColorControlsAfterStartupIfNeeded()", "Startup color controls have a guarded deferred loader"))
    checks.append(require_regex(main_text, r"func loadColorControlsAfterStartupIfNeeded\(\)[\s\S]*self\.loadRecentColorsIfNeeded\(\)[\s\S]*self\.reloadRecentColorRow\(\)[\s\S]*guard !self\.didLoadPaletteGrid else \{ return \}[\s\S]*self\.reloadPaletteGrid\(\)", "Recent colors and palette grid load after startup through a guarded path"))
    checks.append(require_regex(main_text, r"func refreshHistoryUI\([\s\S]*loadDraftThumbnail: Bool = true,[\s\S]*preloadThumbnails: Bool = true,[\s\S]*loadSessions: Bool = true,[\s\S]*checkDraftExistence: Bool = true[\s\S]*if loadSessions[\s\S]*historySessionRefreshGeneration \+= 1[\s\S]*self\.sessionStore\.loadAllSessions\(\)", "Explicit history metadata reloads invalidate stale async metadata results"))
    ...
    checks.append(require_regex(main_text, r"func scheduleStartupDeferredWorkIfNeeded\(\)[\s\S]*scheduleStartupDeferredTask\(after: KCStartupDeferredDelay\.colorControls\)[\s\S]*loadColorControlsAfterStartupIfNeeded\(\)[\s\S]*scheduleStartupDeferredTask\(after: KCStartupDeferredDelay\.restoreDraft\)[\s\S]*restoreDraftIfNeeded\(\)[\s\S]*scheduleStartupDeferredTask\(after: KCStartupDeferredDelay\.historySessions\)[\s\S]*refreshHistorySessionsAsync\(loadDraftThumbnail: false\)", "Startup color controls, draft restore, and history metadata are staggered after first appearance"))
    checks.append(require_regex(main_text, r"func scheduleStartupDeferredWorkIfNeeded\(\)[\s\S]*scheduleStartupDeferredTask\(after: KCStartupDeferredDelay\.stickerButtons\)[\s\S]*loadStickerButtonsAfterStartupIfNeeded\(\)[\s\S]*func scheduleStartupDeferredTask\(after delay: TimeInterval", "Startup sticker button row is deferred to a short staged task"))
    checks.append(require_text(main_text, "static let restoreDraft: TimeInterval = 0.30", "Startup draft restore waits until the first frame has settled"))
    checks.append(require_text(main_text, "static let colorControls: TimeInterval = 0.50", "Startup color controls are delayed to reduce launch-time work"))
    checks.append(require_text(main_text, "static let historySessions: TimeInterval = 0.80", "Startup history metadata waits behind the initial interaction window"))
    checks.append(require_text(main_text, "static let stickerButtons: TimeInterval = 1.10", "Startup sticker buttons are built after heavier startup work has settled"))
    checks.append(require_text(main_text, "controller.refreshHistorySessionsAsync(loadDraftThumbnail: false)", "Startup history refresh does not synchronously decode draft thumbnails"))
    ...
    checks.append(require_regex(main_text, r"func restoreDraftIfNeeded\(\)[\s\S]*self\.canvasView\.restoreCanvas\(with: draftImage\)[\s\S]*self\.refreshHistoryUI\(loadDraftThumbnail: false, loadSessions: false\)", "Startup draft restore refreshes history without rereading draft thumbnails or sessions"))
    checks.append(forbid_regex(main_text, r"func restoreDraftIfNeeded\(\)[\s\S]*self\.sessionStore\.loadDraftImage\(\)", "Startup draft restore no longer synchronously decodes full draft images"))
    checks.append(require_text(main_text, '"openSucceeded": openSucceeded', "Save-history runtime probe records asynchronous open success"))
    ...
    checks.append(require_regex(main_text, r"deinit[\s\S]*self\.flushBrushWidthPreferenceSave\(\)[\s\S]*self\.invalidateDraftSaveTimer\(\)", "Controller deinit flushes pending brush width preferences"))
    ...
    checks.append(require_regex(main_text, r"preserveUnsavedActiveSessionDraftIfNeeded\(\)[\s\S]*self\.clearDraftAndInvalidateCurrentDraftMarker\(\)[\s\S]*self\.canvasView\.loadLineArtImage\(lineArt\)", "Line-art loading preserves dirty edits before replacing the canvas", re.S))
    ...
    checks.append(require_text(sticker_view_presenter_text, "private static let stickerImageCache: NSCache<NSString, UIImage>", "Sticker presenter caches rendered stamp images"))
    checks.append(require_text(sticker_view_presenter_text, "Self.stickerImageCache.object(forKey:", "Sticker presenter reuses cached stamp images"))
    checks.append(require_text(sticker_view_presenter_text, "Self.stickerImageCache.setObject", "Sticker presenter stores rendered stamp images in cache"))
    checks.append(require_text(sticker_view_presenter_text, "rgbaCacheComponent", "Sticker presenter includes normalized color components in cache keys"))
    ...
    checks.append(require_text(history_paging_text, "public func adjacentPageSessionIndexes() -> [Int]", "History paging computes adjacent page preload indexes"))
...
def main():
    ...
    stroke_render_math_text = (ROOT / "Packages" / "KidCanvasModules" / "Sources" / "KCDrawingEngine" / "KCStrokeRenderMath.swift").read_text(encoding="utf-8")
